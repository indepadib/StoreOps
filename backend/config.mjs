...
export const config = {
  ...
  dynamics: {
    ...
    transferReceiving: {
      headerEntity: process.env.D365_TO_HEADER_ENTITY || 'TransferOrderHeaders',
      lineEntity: process.env.D365_TO_LINE_ENTITY || 'TransferOrderLinesV2',
      numberField: process.env.D365_TO_NUMBER_FIELD || 'TransferOrderNumber',
      statusField: process.env.D365_TO_STATUS_FIELD || 'TransferOrderStatus',
      fromWarehouseField: process.env.D365_TO_FROM_WAREHOUSE_FIELD || 'ShippingWarehouseId',
      toWarehouseField: process.env.D365_TO_TO_WAREHOUSE_FIELD || 'ReceivingWarehouseId',
      headerDateField: process.env.D365_TO_RECEIPT_DATE_FIELD || 'RequestedReceiptDate',
      lineNumberField: process.env.D365_TO_LINE_NUMBER_FIELD || 'LineNumber',
      productField: process.env.D365_TO_PRODUCT_FIELD || 'ItemNumber',
      transferQtyField: process.env.D365_TO_TRANSFER_QTY_FIELD || 'TransferQuantity',
      receivedQtyField: process.env.D365_TO_RECEIVED_QTY_FIELD || 'ReceivedQuantity',
      remainingQtyField: process.env.D365_TO_REMAINING_QTY_FIELD || 'RemainingReceivedQuantity',
      unitField: process.env.D365_TO_UNIT_FIELD || 'InventoryUnitSymbol',
      lineDateField: process.env.D365_TO_LINE_DATE_FIELD || 'RequestedReceiptDate',
      pageSize: Math.max(50,Math.min(1000,Number(process.env.D365_TO_PAGE_SIZE)||250)),
      maxRows: Math.max(500,Math.min(25000,Number(process.env.D365_TO_MAX_ROWS)||10000))
    },
    receiving: {
      ...
      lineStatusField: process.env.D365_PO_LINE_STATUS_FIELD || 'PurchaseOrderLineStatus',
      receivedQtyField: process.env.D365_PO_RECEIVED_QTY_FIELD || '',
      remainingQtyField: process.env.D365_PO_REMAINING_QTY_FIELD || '',
