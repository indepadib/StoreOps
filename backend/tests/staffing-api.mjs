...
x=await call('GET','/api/stores/val-fleuri/staffing','u-vf');ok(x.r.status===200&&!x.data.day&&x.data.sync?.code==='STAFFING_NOT_SYNCED','staffing GET must remain read-only before first sync');x=await call('POST','/api/stores/val-fleuri/staffing/sync','u-vf');ok(x.r.status===200&&x.data.day?.lines?.length===4,'staffing explicit sync failed');const lines=x.data.day.lines,mgr=lines.find(i=>i.role_code==='MANAGER'),floor=lines.find(i=>i.role_code==='FLOOR'),cash=lines.filter(i=>i.role_code==='CASHIER');
