...
assert.match(fn,/pg_advisory_xact_lock/,'public backend must serialize SQLite writes/snapshot refreshes');
assert.match(fn,/storeops_sqlite_state/,'public backend must persist the SQLite state centrally');
assert.match(fn,/READ_METHODS=new Set\(\['GET','HEAD','OPTIONS'\]\)/,'read-only methods must be explicit');
assert.match(fn,/X-StoreOps-Bridge/,'public backend must expose its fast-read/write-sync mode for diagnostics');
const readBlock=fn.match(/async function serveRead[\s\S]*?async function serveWrite/)?.[0]||'';
const writeBlock=fn.match(/async function serveWrite[\s\S]*?export default/)?.[0]||'';
assert(readBlock,'serveRead block missing');
assert(writeBlock,'serveWrite block missing');
assert.doesNotMatch(readBlock,/persistSnapshot\(/,'GET/HEAD/OPTIONS must never rewrite the central SQLite snapshot');
assert.doesNotMatch(readBlock,/pg_advisory_xact_lock/,'warm fast reads must not take the global advisory lock');
assert.match(writeBlock,/pg_advisory_xact_lock/,'writes must keep the global advisory lock');
assert.match(writeBlock,/persistSnapshot\(/,'writes must persist the central SQLite snapshot');
...
console.log('StoreOps public backend fast-read + durable-write contract OK');
