...
let s=syncStaffingDay({storeId:'val-fleuri',businessDate:date,snapshot}),initial=staffingSummary('val-fleuri',date);ok(s.lines.length===4&&initial.blocking>0&&initial.pending===4,'staffing sync failed');
...
let p=updateStaffingPolicy({user:director,requiredManagers:1,requiredCashiers:2,requiredFloor:1});ok(Number(p.required_cashiers)===2,'staffing policy update failed');ok(staffingSummary('val-fleuri',date).coverageOk===false,'higher cashier minimum must recalc coverage');updateStaffingPolicy({user:director,requiredManagers:1,requiredCashiers:1,requiredFloor:1});
