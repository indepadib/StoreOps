...
assert.equal(sim.length,15);
assert.equal(sim.find(x=>x.key==='receiving').read.code,'MAPPING');
assert.equal(sim.find(x=>x.key==='receiving').write.code,'MAPPING');
assert.equal(sim.find(x=>x.key==='assortment').read.code,'MAPPING');
assert.equal(sim.find(x=>x.key==='taxonomy').read.code,'MAPPING');
assert.equal(sim.find(x=>x.key==='sales').read.code,'MAPPING');
assert.equal(sim.find(x=>x.key==='supply').read.code,'MAPPING');
...
assert.notEqual(sim.find(x=>x.key==='product').read.code,'LIVE','Simulated mode must never claim connector live');
...
assert.notEqual(live.find(x=>x.key==='assortment').read.code,'LIVE');
assert.notEqual(live.find(x=>x.key==='sales').read.code,'LIVE');
...
console.log('StoreOps integration readiness contract OK');
