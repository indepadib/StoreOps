...
assert.ok(app.includes("document.querySelectorAll('#nav button[data-page],#managerNav button[data-page],#qualityAuditNav button[data-page],#developmentNavBar button[data-page]').forEach"),'navigation binding must use a true multi-element selector before forEach');
