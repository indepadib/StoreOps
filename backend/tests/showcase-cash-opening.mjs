...
const users={
 'u-vf':{id:'u-vf',name:'Responsable Val Fleuri',role:'store_manager',store_id:'val-fleuri'},
...
let x=await mockCashOpeningApi('/api/stores/val-fleuri/cash-opening',{},core);ok(x.opening.lines.length===3&&x.summary.blocking===3,'showcase cash opening snapshot failed');const [l1,l2,l3]=x.opening.lines;
...
for(const [line,name] of [[l1,'Sara'],[l2,'Yassine'],[l3,'Imane']])await mockCashOpeningApi(`/api/cash-opening/lines/${line.id}/check`,{method:'POST',body:JSON.stringify({cashierName:name,declaredFloat:500,posOk:true,tpeOk:true,printerOk:true,shiftOpened:true})},core);
...
console.log('StoreOps V1.11 showcase cash opening tests passed');
