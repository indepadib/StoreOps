...
ok(x.opening.lines.every(l=>Number(l.expected_float)===1000)&&Number(x.summary.expectedFloat)===2000,'Val Fleuri opening float must be 1000 DH per till');
localStorage.setItem('storeops_user','u-emp-vf');let denied=false;try{await mockCashOpeningApi(`/api/cash-opening/lines/${l1.id}/check`,{method:'POST',body:JSON.stringify({cashierName:'Employé',declaredFloat:1000,posOk:true,tpeOk:true,printerOk:true,shiftOpened:true})},core)}catch(e){denied=e.status===403}ok(denied,'showcase employee cash opening permission failed');
localStorage.setItem('storeops_user','u-vf');let mismatch=false;try{await mockCashOpeningApi(`/api/cash-opening/lines/${l1.id}/check`,{method:'POST',body:JSON.stringify({cashierName:'Sara',declaredFloat:950,posOk:true,tpeOk:true,printerOk:true,shiftOpened:true})},core)}catch(e){mismatch=e.status===409}ok(mismatch&&cashOpeningShowcaseSummary('val-fleuri').mismatch===1,'showcase cash opening mismatch failed');
for(const [line,name] of [[l1,'Sara'],[l2,'Yassine']])await mockCashOpeningApi(`/api/cash-opening/lines/${line.id}/check`,{method:'POST',body:JSON.stringify({cashierName:name,declaredFloat:1000,posOk:true,tpeOk:true,printerOk:true,shiftOpened:true})},core);
...
let locked=false;try{await mockCashOpeningApi(`/api/cash-opening/lines/${l1.id}/check`,{method:'POST',body:JSON.stringify({cashierName:'Sara',declaredFloat:1000,posOk:true,tpeOk:true,printerOk:true,shiftOpened:true})},core)}catch(e){locked=e.status===409}ok(locked,'showcase cash opening must lock after opening');
