...
assert.match(sw,/req\.mode===['"]navigate['"][\s\S]*fetch\(req,\{cache:['"]no-store['"]\}\)/,'navigation must always fetch fresh HTML');
...
assert.ok(!assets.includes('/')&&!assets.includes('/index.html'),'HTML shell must not be precached during pilot hardening');
...
assert.doesNotMatch(bootClassic,/if\(alreadyClean\(\)\)return/,'legacy cleanup must run on every boot');
