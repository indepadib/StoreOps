...
const {syncCashOpening,cashOpeningSummary,checkCashOpeningLine,markCashOpeningOpened,updateCashOpeningPolicy}=await import('../services/cash-opening.mjs');
function ok(v,m){if(!v)throw new Error(m)}
const manager=db.prepare(`SELECT * FROM users WHERE id='u-vf'`).get(),director=db.prepare(`SELECT * FROM users WHERE id='u-ops'`).get(),date=new Date().toISOString().slice(0,10);
ensureStoreDay('val-fleuri',date);
...
const day=db.prepare(`SELECT id FROM store_days WHERE store_id='val-fleuri' AND business_date=?`).get(date),task=db.prepare(`SELECT * FROM tasks WHERE store_day_id=? AND group_name='opening' AND step_order=7`).get(day.id);ok(task.status==='COMPLETED','opening cash task must auto-complete');
opening=markCashOpeningOpened({storeId:'val-fleuri',businessDate:date,user:manager});ok(opening.status==='OPENED','cash opening OPENED transition failed');
