...
async function api(req,res,url){
  ...
  if(path==='/api/health')return json(req,res,200,{ok:true,service:'StoreOps API',version:'1.4.4',authMode:config.authMode,dynamicsMode:config.dynamics.mode,configurationIssues:productionMisconfig()});
  ...
  if(path==='/api/config')return json(req,res,200,{authMode:config.authMode,dynamicsMode:config.dynamics.mode,version:'1.4.4'});
  ...
  p=route(path,'/api/stores/:storeId/handover');if(p){
    ...
    if(req.method==='GET'){const businessDate=url.searchParams.get('date')||todayISO(),status=url.searchParams.get('status')||'ACTIVE';return json(req,res,200,{stats:handoverStats(p.storeId,businessDate),items:listHandover(p.storeId,{businessDate,status})})}
  ...
  p=route(path,'/api/stores/:storeId/dashboard');if(p){
    ...
    const dlcs=db.prepare(`SELECT * FROM dlc_records WHERE store_id=? AND status='ACTIVE' ORDER BY expiry_date`).all(p.storeId).map(x=>({...x,risk:dlcStage(x.expiry_date)})),incidentSummary=incidentStats(p.storeId),handover=handoverStats(p.storeId,day.business_date),quality=db.prepare(`SELECT COUNT(*) n,COALESCE(SUM(rejected_qty),0) rejected FROM quality_controls WHERE store_id=? AND date(created_at)=?`).get(p.storeId,day.business_date);
    return json(req,res,200,{day,cycle:dayCycleMetrics(day),opening,closing,handover,dlcAtRisk:dlcs.filter(x=>x.risk.stage!=='OK').length,incidents:incidentSummary.open,criticalIncidents:incidentSummary.critical,overdueIncidents:incidentSummary.overdue,escalatedIncidents:incidentSummary.escalated,watchIncidents:incidentSummary.watch,qualityControls:quality.n,qualityRejected:quality.rejected,health:Math.max(0,100-incidentSummary.open*6-incidentSummary.escalated*8-handover.blocking*6-opening.blockers*4),lastActions:db.prepare(`SELECT a.*,u.name actor FROM audit_log a LEFT JOIN users u ON u.id=a.user_id WHERE a.store_id=? ORDER BY a.id DESC LIMIT 12`).all(p.storeId)})
  }
...
server.listen(PORT,()=>console.log(`StoreOps V1.4.4 running on http://localhost:${PORT} · auth=${config.authMode} · dynamics=${config.dynamics.mode}`));
