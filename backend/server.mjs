...
async function api(req,res,url){
  ...
  p=route(path,'/api/tasks/:taskId/submit'); if(p && req.method==='POST'){
    const form=getTaskForm(p.taskId);if(!form)return json(req,res,404,{error:'Tâche introuvable'});requireStore(user,form.task.store_id);ensureManage(user,form.task.store_id);const b=await body(req);const result=submitTaskForm({taskId:p.taskId,user,values:b.values||{}});return json(req,res,result.ok?200:409,result)
  }
  p=route(path,'/api/stores/:storeId/process/:group/take'); if(p && req.method==='POST'){requireStore(user,p.storeId);ensureManage(user,p.storeId);const day=ensureStoreDay(p.storeId);takeOwnership({storeDay:day,user,group:p.group});return json(req,res,200,{ok:true})}
  p=route(path,'/api/stores/:storeId/process/:group/validate'); if(p && req.method==='POST'){requireStore(user,p.storeId);ensureManage(user,p.storeId);const day=ensureStoreDay(p.storeId);const progress=validateProcess({storeDay:day,user,group:p.group});return json(req,res,200,{ok:true,progress})}

  p=route(path,'/api/stores/:storeId/dlc'); if(p){
    requireStore(user,p.storeId);
    if(req.method==='GET'){const rows=db.prepare(`SELECT d.*,u.name created_by_name FROM dlc_records d JOIN users u ON u.id=d.created_by WHERE d.store_id=? ORDER BY d.expiry_date`).all(p.storeId).map(x=>({...x,risk:dlcStage(x.expiry_date)}));return json(req,res,200,rows)}
    if(req.method==='POST'){ensureManage(user,p.storeId);const b=await body(req);const product=await getProductByEan(b.ean);if(!product)return json(req,res,400,{error:'EAN inconnu Dynamics'});if(!b.expiryDate||!(Number(b.quantity)>0))return json(req,res,400,{error:'DLC et quantité obligatoires'});const id=uid('dlc');db.prepare(`INSERT INTO dlc_records(id,store_id,ean,product_name,expiry_date,quantity,zone,lot_ref,comment,created_by) VALUES(?,?,?,?,?,?,?,?,?,?)`).run(id,p.storeId,b.ean,product.name,b.expiryDate,Number(b.quantity),b.zone||'Rayon',b.lotRef||null,b.comment||null,user.id);audit({storeId:p.storeId,userId:user.id,action:'DLC_CREATED',entityType:'DLC',entityId:id,details:{ean:b.ean,expiryDate:b.expiryDate,quantity:b.quantity}});return json(req,res,201,{id,product})}
  }

  p=route(path,'/api/stores/:storeId/quality'); if(p){
    requireStore(user,p.storeId);
    if(req.method==='GET'){const rows=db.prepare(`SELECT q.*,u.name controlled_by_name FROM quality_controls q JOIN users u ON u.id=q.controlled_by WHERE q.store_id=? ORDER BY q.created_at DESC`).all(p.storeId);return json(req,res,200,rows)}
    if(req.method==='POST'){
      ensureQuality(user,p.storeId);const b=await body(req);const delivered=Number(b.deliveredQty),accepted=Number(b.acceptedQty),rejected=Number(b.rejectedQty);
      if(!Number.isFinite(delivered)||!Number.isFinite(accepted)||!Number.isFinite(rejected)||delivered<0||accepted<0||rejected<0)return json(req,res,400,{error:'Quantités invalides'});
      if(Math.abs(accepted+rejected-delivered)>0.0001)return json(req,res,400,{error:'Accepté + refusé doit être égal au livré'});
      const product=await getProductByEan(b.ean);if(!product)return json(req,res,400,{error:'EAN inconnu Dynamics'});
      const evaluation=evaluateQuality({product,temperature:b.temperature,packagingStatus:b.packagingStatus||'NA',appearanceStatus:b.appearanceStatus||'NA',expiryDate:b.expiryDate||null});
      if(evaluation.issues.length && rejected===0)return json(req,res,409,{error:'Contrôle non conforme : une quantité refusée ou une correction est nécessaire.',issues:evaluation.issues,qualityProfile:evaluation.profile});
  ...
  p=route(path,'/api/stores/:storeId/receipts'); if(p){requireStore(user,p.storeId);const rows=db.prepare(`SELECT * FROM receipts WHERE store_id=? ORDER BY eta`).all(p.storeId).map(r=>({...r,lines:db.prepare(`SELECT * FROM receipt_lines WHERE receipt_id=?`).all(r.id)}));return json(req,res,200,rows)}
  p=route(path,'/api/receipts/:po/lines/:lineId/quality'); if(p && req.method==='POST'){
    const r=db.prepare(`SELECT * FROM receipts WHERE po_number=?`).get(p.po);if(!r)return json(req,res,404,{error:'PO introuvable'});requireStore(user,r.store_id);ensureQuality(user,r.store_id);
    const line=db.prepare(`SELECT * FROM receipt_lines WHERE id=? AND receipt_id=?`).get(p.lineId,r.id);if(!line)return json(req,res,404,{error:'Ligne introuvable'});const b=await body(req);
    const delivered=Number(b.deliveredQty),accepted=Number(b.acceptedQty),rejected=Number(b.rejectedQty);if(!Number.isFinite(delivered)||!Number.isFinite(accepted)||!Number.isFinite(rejected)||Math.abs(accepted+rejected-delivered)>0.0001)return json(req,res,400,{error:'Accepté + refusé doit être égal au livré'});
    const product={ean:line.ean,name:line.product_name,category:line.category||'Autre'};const evaluation=evaluateQuality({product,temperature:b.temperature,packagingStatus:b.packagingStatus||'NA',appearanceStatus:b.appearanceStatus||'NA',expiryDate:b.expiryDate||null});
    if(evaluation.issues.length && rejected===0)return json(req,res,409,{error:'Non-conformité détectée : renseigner la quantité refusée.',issues:evaluation.issues,qualityProfile:evaluation.profile});
    const decision=rejected===0?'ACCEPT':accepted===0?'REJECT':'PARTIAL';const id=uid('qc');
  ...
  p=route(path,'/api/receipts/:po/post'); if(p && req.method==='POST'){
    const r=db.prepare(`SELECT * FROM receipts WHERE po_number=?`).get(p.po);if(!r)return json(req,res,404,{error:'PO introuvable'});requireStore(user,r.store_id);ensureQuality(user,r.store_id);
    const lines=db.prepare(`SELECT * FROM receipt_lines WHERE receipt_id=?`).all(r.id);if(lines.some(l=>!l.quality_control_id))return json(req,res,409,{error:'Toutes les lignes doivent avoir un contrôle qualité avant réception système'});
    const dyn=await postReceiptToDynamics(r.po_number,{lines});db.prepare(`UPDATE receipts SET status='POSTED',posted_at=CURRENT_TIMESTAMP WHERE id=?`).run(r.id);audit({storeId:r.store_id,userId:user.id,action:'RECEIPT_POSTED',entityType:'RECEIPT',entityId:r.id,details:dyn});return json(req,res,200,dyn);
  }
...
function staticFile(req,res,url){
  let path=url.pathname==='/'?'/index.html':url.pathname;path=normalize(path).replace(/^\.\.(\/|\\|$)/,'');const file=join(FRONTEND,path);
  if(!file.startsWith(FRONTEND)||!existsSync(file)){res.writeHead(404);return res.end('Not found')}
  const type={'.html':'text/html','.js':'text/javascript','.mjs':'text/javascript','.css':'text/css','.svg':'image/svg+xml','.png':'image/png','.json':'application/json'}[extname(file)]||'application/octet-stream';res.writeHead(200,{'content-type':type,'cache-control':path==='/index.html'?'no-cache':'public, max-age=3600'});res.end(readFileSync(file));
}
