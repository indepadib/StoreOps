...
async function api(req,res,url){
  ...
  p=route(path,'/api/stores/:storeId/dashboard');if(p){
    ...
    const dlc=dlcSummary(p.storeId),incidentSummary=incidentStats(p.storeId),quality=db.prepare(`SELECT COUNT(*) n,COALESCE(SUM(rejected_qty),0) rejected FROM quality_controls WHERE store_id=? AND date(created_at)=?`).get(p.storeId,day.business_date);
    return json(req,res,200,{day,opening,closing,dlc,dlcAtRisk:dlc.expired+dlc.critical+dlc.alert+dlc.watch,incidents:incidentSummary.open,criticalIncidents:incidentSummary.critical,overdueIncidents:incidentSummary.overdue,escalatedIncidents:incidentSummary.escalated,watchIncidents:incidentSummary.watch,qualityControls:quality.n,qualityRejected:quality.rejected,health:Math.max(0,100-incidentSummary.open*6-incidentSummary.escalated*8-dlc.expired*8-dlc.critical*5-opening.blockers*4),lastActions:db.prepare(`SELECT a.*,u.name actor FROM audit_log a LEFT JOIN users u ON u.id=a.user_id WHERE a.store_id=? ORDER BY a.id DESC LIMIT 12`).all(p.storeId)})
  }
...
server.listen(PORT,()=>console.log(`StoreOps V1.5 DLC running on http://localhost:${PORT} · auth=${config.authMode} · dynamics=${config.dynamics.mode}`));
