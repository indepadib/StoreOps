...
const action=({id,category,severity='HIGH',title,detail,page,blocking=false,meta='',priority='P2',source='',stockFlow=null,count=1})=>({id,category,severity,title,detail,page,blocking:!!blocking,meta,priority,source,stockFlow,count});
...
async function computeManagerInboxBatch(storeId,businessDate,{force=false}={}){
 ...
 const negativeSignals=stockSignals.filter(sig=>sig.type==='NEGATIVE'||n(sig.qty)<0),outSignals=stockSignals.filter(sig=>sig.type==='OUT'),promoOut=outSignals.filter(sig=>promoEans.has(String(sig.ean||'')));
 if(negativeSignals.length)items.push(action({id:'stock-negative-batch',category:'STOCK',severity:'CRITICAL',title:`Stocks négatifs · ${negativeSignals.length} article${negativeSignals.length>1?'s':''}`,detail:`Parcours guidé article par article · contrôle physique puis inventaire ciblé`,page:'inventory',meta:'Anomalies stock',priority:'P0',source:stockData.source||'',stockFlow:'NEGATIVE',count:negativeSignals.length}));
 if(outSignals.length)items.push(action({id:'stock-oos-batch',category:'STOCK',severity:promoOut.length?'CRITICAL':'HIGH',title:`Ruptures · ${outSignals.length} article${outSignals.length>1?'s':''}`,detail:`${promoOut.length?`${promoOut.length} rupture${promoOut.length>1?'s':''} promo · `:''}parcours guidé rayon → réserve → confirmation`,page:'inventory',meta:'Ruptures assortiment',priority:promoOut.length?'P0':'P1',source:stockData.source||'',stockFlow:'OUT',count:outSignals.length}));
 const unknownZero=n(stockData.summary?.assortmentUnknownZero);if(unknownZero&&!outSignals.length)items.push(action({id:'stock-assortment-unknown',category:'STOCK',severity:'HIGH',title:`Stock à zéro non qualifié · ${unknownZero} article${unknownZero>1?'s':''}`,detail:'Assortiment inconnu ou périmé : StoreOps refuse de les appeler ruptures tant que la donnée assortiment n’est pas fiable.',page:'inventory',meta:'Donnée assortiment',priority:'P1',source:stockData.source||''}));
