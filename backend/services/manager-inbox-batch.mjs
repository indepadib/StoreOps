...
async function computeManagerInboxBatch(storeId,businessDate,{force=false}={}){
 ...
 if(ruptureCount>0)items.push(action({id:'stock-rupture-summary',category:'STOCK',severity:promoPreview.length?'CRITICAL':'HIGH',title:ruptureCount+' rupture(s) à contrôler',detail:(topNames(ruptureSignals)||'Articles vendus sur 30j')+(ruptureCount>ruptureSignals.length?' · +'+(ruptureCount-ruptureSignals.length)+' autre(s)':''),page:'inventory',meta:promoPreview.length?promoPreview.length+' promo(s) dans l’aperçu':'Vendus 30j · stock 0',priority:promoPreview.length?'P0':'P1',source:stockData.source||''}));
 if(residualSignals.length)items.push(action({id:'stock-residual-summary',category:'STOCK',severity:'NORMAL',title:(n(stockData.summary?.residualOutsideAssortment)||residualSignals.length)+' stock(s) hors assortiment',detail:(topNames(residualSignals)||'À traiter')+' · détail disponible dans le contrôle stock',page:'inventory',meta:'À surveiller',priority:'P3',source:stockData.source||''}));
