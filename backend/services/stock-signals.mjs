...
function simulated(storeId){
  if(storeId!=='val-fleuri')return {source:'SIMULATED',storeId,warehouse:null,items:[],summary:{total:0,negative:0,outOfStock:0},checkedAt:new Date().toISOString()};
  const items=[
    {id:'sim-neg-coca',type:'NEGATIVE',priority:'P0',product:'Coca-Cola 1,5L',productNumber:'COCA15',ean:'5449000000996',qty:-3,availableQty:-3,physicalQty:-3,detail:'Stock disponible -3 · vérifier rayon + réserve puis lancer un inventaire ciblé'},
    {id:'sim-oos-eau',type:'OUT',priority:'P1',product:'Sidi Ali 1,5L',productNumber:'EAU15',ean:'6111000000011',qty:0,availableQty:0,physicalQty:0,detail:'Rupture détectée · stock disponible 0 · contrôler réserve / facing'},
    {id:'sim-oos-lait',type:'OUT',priority:'P1',product:'Lait UHT entier 1L',productNumber:'LAITUHT1',ean:'6111035000013',qty:0,availableQty:0,physicalQty:0,detail:'Rupture détectée · stock disponible 0 · contrôler disponibilité physique'}
  ];
  return {source:'SIMULATED',storeId,warehouse:'VF-DEMO',items,summary:{total:items.length,negative:1,outOfStock:2},checkedAt:new Date().toISOString()};
}
...
function aggregateRows(rows,c){
  ...
  for(const row of rows||[]){
    ...
    const current=byProduct.get(key)||{productNumber,product:clean(c.nameField?row[c.nameField]:'')||productNumber,ean:clean(c.eanField?row[c.eanField]:'')||null,availableQty:0,physicalQty:0,rowCount:0};
...
export async function getStockSignals(storeId){
  ...
  const entity=clean(c.entity);if(!entity)throw Object.assign(new Error('D365_STOCK_ENTITY non configuré'),{status:503,code:'D365_STOCK_MAPPING_REQUIRED'});
  ...
  const warehouse=clean(c.storeWarehouses?.[storeId]);
  ...
  const productField=requireField('D365_STOCK_PRODUCT_FIELD',c.productField);
  const availableField=requireField('D365_STOCK_AVAILABLE_FIELD',c.availableField);
  const warehouseField=requireField('D365_STOCK_WAREHOUSE_FIELD',c.warehouseField);
  const physicalField=clean(c.physicalField);if(physicalField&&!validField(physicalField))throw Object.assign(new Error('D365_STOCK_PHYSICAL_FIELD invalide'),{status:503,code:'D365_STOCK_MAPPING_INVALID'});
  ...
  return {source:'D365',storeId,warehouse,checkedAt:new Date().toISOString(),entity,items,summary:{total:items.length,negative:negative.length,outOfStock:out.length,aggregatedProducts:aggregated.length,rowsRead:fetched.rowCount,pages:fetched.pages,truncated:!!fetched.truncated}};
}
