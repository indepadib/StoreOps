...
export async function getRetailPromotionsByItem(productNumber,{businessDate=null,priceGroup=null}={}){
  ...
  // Critical production optimization: let D365 filter on ItemId. Never download the full RetailDiscountLines entity.
  const lineFilters=[`ItemId eq '${escapeOData(item)}'`];
  if(config.dynamics.dataAreaId)lineFilters.push(`${config.dynamics.dataAreaField} eq '${escapeOData(config.dynamics.dataAreaId)}'`);
  const linePayload=await odataGetAllBySkip(entity('retailDiscountLine',RETAIL_DISCOUNT_LINE_ENTITY),{
    filter:lineFilters.join(' and '),
  ...
  if(!offerIds.length)return{mode:'LIVE',productNumber:item,businessDate:day,priceGroup:resolvedPriceGroup,promotions:[],rowCount:0,coverage:{directItem:true,category:false},scan:{rowsScanned:lines.length,pages:linePayload.pages||1,truncated:!!linePayload.truncated,filteredByItem:true}};
  ...
  const [headersPayload,groupsPayload,mixPayload]=await Promise.all([
    ...
    odataGet(entity('mixMatchLineGroup',MIX_MATCH_LINE_GROUP_ENTITY),{filter:withCompany(orFilter('MixAndMatchOfferId',offerIds)),...common})
  ]);
  const headers=Array.isArray(headersPayload?.value)?headersPayload.value:[],groups=Array.isArray(groupsPayload?.value)?groupsPayload.value:[],mixGroups=Array.isArray(mixPayload?.value)?mixPayload.value:[];
  ...
  const promotions=offerIds.map(offerId=>{
    ...
    const mechanic=mechanicFor(header,line,mixGroup),status=promotionStatus(header,day),priceGroupEligible=!resolvedPriceGroup||priceGroups.length===0||priceGroups.includes(resolvedPriceGroup);
    return{offerId,name:header?.Name||line?.Name||offerId,periodicDiscountType:header?.PeriodicDiscountType||null,status,enabled:header?.Status==='Enabled',processingStatus:header?.ProcessingStatus||null,validFrom:header?.ValidFrom||null,validTo:header?.ValidTo||null,currencyCode:header?.CurrencyCode||null,concurrencyMode:header?.ConcurrencyMode||null,pricingPriorityNumber:header?.PricingPriorityNumber??null,priceGroups,priceGroupEligible,itemLine:{lineNum:line?.LineNum??null,itemId:line?.ItemId||item,name:line?.Name||null,unit:line?.UnitOfMeasureSymbol||null,lineType:line?.LineType||null,mixAndMatchLineGroup:line?.MixAndMatchLineGroup||null},mechanic,activeForRequestedContext:status==='ACTIVE'&&priceGroupEligible,rawLineCount:offerLines.length};
  });
  return{mode:'LIVE',productNumber:item,businessDate:day,priceGroup:resolvedPriceGroup,promotions,rowCount:promotions.length,coverage:{directItem:true,category:false},scan:{rowsScanned:lines.length,pages:linePayload.pages||1,truncated:!!linePayload.truncated,filteredByItem:true}};
}
...
export async function getProductPricing(productNumber,{businessDate=null,priceGroup=null}={}){
  ...
  const [priceDiagnostics,promoDiagnostics]=await Promise.all([getSalesPriceAgreementsByItem(item),getRetailPromotionsByItem(item,{businessDate,priceGroup:resolvedPriceGroup})]);
