...
export async function handleLossApi({req,url,user}){
 ...
 p=route(path,'/api/stores/:storeId/staffing');if(p&&req.method==='GET'){
  ...
  try{const snapshot=await getStaffingSnapshot(p.storeId,businessDate);sync={ok:true,day:syncStaffingDay({storeId:p.storeId,businessDate,snapshot})}}
  catch(e){sync={ok:false,error:e.message,code:e.code||'STAFFING_SYNC_FAILED'}}
  return{status:200,data:{summary:staffingSummary(p.storeId,businessDate),day:staffingDay(p.storeId,businessDate),sync}};
 }
