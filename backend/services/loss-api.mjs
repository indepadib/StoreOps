...
export async function handleLossApi({req,url,user}){
 ...
 p=route(path,'/api/stores/:storeId/cold-chain');if(p&&req.method==='GET'){requireStore(user,p.storeId);const businessDate=url.searchParams.get('date')||todayISO(),day=coldChainDay(p.storeId,businessDate);return{status:200,data:{summary:coldChainSummary(p.storeId,businessDate),day,sync:day?{ok:true,cached:true}:{ok:false,code:'COLD_CHAIN_NOT_SYNCED',message:'Contrôle froid non initialisé pour cette journée.'}}}}
 p=route(path,'/api/stores/:storeId/cold-chain/sync');if(p&&req.method==='POST'){requireStore(user,p.storeId);requireQualityManage(user,p.storeId);const businessDate=url.searchParams.get('date')||todayISO(),day=ensureColdChainDay(p.storeId,businessDate);return{status:200,data:{summary:coldChainSummary(p.storeId,businessDate),day,sync:{ok:true,source:'STOREOPS'}}}}
 ...
 p=route(path,'/api/stores/:storeId/cash-opening');if(p&&req.method==='GET'){requireStore(user,p.storeId);const businessDate=url.searchParams.get('date')||todayISO(),opening=cashOpening(p.storeId,businessDate);return{status:200,data:{summary:cashOpeningSummary(p.storeId,businessDate),opening,sync:opening?{ok:true,cached:true}:{ok:false,code:'CASH_OPENING_NOT_SYNCED',message:'Préparation caisses non synchronisée pour cette journée.'}}}}
