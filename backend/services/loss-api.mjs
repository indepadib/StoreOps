...
export async function handleLossApi({req,url,user}){
 ...
 p=route(path,'/api/stores/:storeId/losses');if(p){requireStore(user,p.storeId);const businessDate=url.searchParams.get('date')||todayISO();if(req.method==='GET'){const status=(url.searchParams.get('status')||'ALL').toUpperCase();return{status:200,data:{summary:lossSummary(p.storeId,businessDate),items:listLossRecords(p.storeId,businessDate,status)}}}if(req.method==='POST'){requireManage(user,p.storeId);const b=await body(req),product=await getStoreCommerceProduct(p.storeId,String(b.ean||'').trim(),businessDate);if(!product)throw Object.assign(new Error('Article introuvable Dynamics.'),{status:404});let record=createLossRecord({storeId:p.storeId,businessDate,user,product,reasonCode:b.reasonCode,quantity:b.quantity,unit:b.unit,note:b.note,sourceType:b.sourceType||'MANUAL',sourceId:b.sourceId||null}),incidentResolved=false;if(b.evidenceDataUrl){let incident=record.incident_id?incidentById(record.incident_id):null;if(!incident){incident=createIncident({storeId:p.storeId,user,title:`Preuve démarque · ${record.product_name}`,description:`${record.quantity} ${record.unit} · ${record.reason_code}`,category:'LOSS',criticality:'LOW',blockingLevel:'NONE',sourceType:'LOSS_RECORD',sourceId:record.id,assignedTo:user.role==='store_manager'?user.id:null,requiresEvidence:true});db.prepare(`UPDATE loss_records SET incident_id=? WHERE id=?`).run(incident.id,record.id)}addEvidence({incidentId:incident.id,user,dataUrl:b.evidenceDataUrl,fileName:b.evidenceFileName||'preuve-demarque.jpg',caption:b.evidenceCaption||'Preuve terrain démarque'});const hydrated=incidentById(incident.id);for(const a of hydrated.actions.filter(x=>x.status==='OPEN'))completeAction({incidentId:incident.id,actionId:a.id,user,note:'Preuve terrain jointe lors de la saisie de démarque'});if(incidentById(incident.id).status==='OPEN')resolveIncident({incidentId:incident.id,user,resolutionNote:'Perte documentée au moment de la saisie avec preuve photo.'});db.prepare(`UPDATE loss_records SET evidence_satisfied=1,evidence_source_type='INCIDENT_EVIDENCE',evidence_source_id=? WHERE id=?`).run(incident.id,record.id);incidentResolved=true;record=lossRecord(record.id)}return{status:201,data:{...record,incidentResolved}}}}
