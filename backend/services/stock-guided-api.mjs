...
import { getStockGuidedFlow,completeStockGuidedReview,stockGuidedConfig } from './stock-guided.mjs';
...
export async function handleStockGuidedApi({req,url,user}){
 ...
 let p=route(path,'/api/stores/:storeId/stock-guided/:type');
 if(p&&req.method==='GET'){requireStore(user,p.storeId);return{status:200,data:await getStockGuidedFlow(p.storeId,p.type,{businessDate:url.searchParams.get('date')||undefined,force:url.searchParams.get('force')==='1'})}}
