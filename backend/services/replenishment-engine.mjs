...
export function recommendReplenishment(input={}){
 const storeKnown=known(input.storeAvailable),supplyKnown=known(input.supplyAvailable);
 const storeAvailable=storeKnown?num(input.storeAvailable,0):null,supplyAvailable=supplyKnown?Math.max(0,num(input.supplyAvailable,0)):null,confirmedInbound=Math.max(0,num(input.confirmedInbound,0));
 ...
 const adjustedVelocity=velocity===null?null:round3(velocity*promoFactor*dayFactor),coverDays=storeKnown&&adjustedVelocity&&adjustedVelocity>0?round3(Math.max(0,storeAvailable)/adjustedVelocity):null;
 const targetDays=round3(leadTimeDays+safetyDays),targetDemand=adjustedVelocity===null?null:round3(adjustedVelocity*targetDays),rawNeed=!storeKnown||targetDemand===null?null:round3(targetDemand-storeAvailable-confirmedInbound);
 ...
 return{
  ...
  actionQty,
  reason,
  inputs:{storeAvailable:storeKnown?round3(storeAvailable):null,supplyAvailable:supplyKnown?round3(supplyAvailable):null,confirmedInbound:round3(confirmedInbound),dailySales7:input.dailySales7??null,dailySales28:input.dailySales28??null,leadTimeDays,safetyDays,promoFactor,dayOfWeekFactor:dayFactor,packSize,minOrderQty},
  ...
  explanation:[
   !storeKnown?'Stock magasin indisponible':null,
   ...
   supplyKnown?`Entrepôt disponible ${round3(supplyAvailable)}`:'Stock entrepôt/source non connecté'
  ].filter(Boolean)
 }
}
