...
function categoryOptions(){return db.prepare(`SELECT category_id id,category_name name,level,path,hierarchy_key hierarchyKey FROM merchandising_categories WHERE active=1 ORDER BY COALESCE(level,999),category_name LIMIT 5000`).all()}

export async function handleReplenishmentPolicyApi({req,url,user}){
 const path=url.pathname;let p;
 if(path==='/api/admin/replenishment-rules/options'&&req.method==='GET'){director(user);return{status:200,data:{categories:categoryOptions()}}}
