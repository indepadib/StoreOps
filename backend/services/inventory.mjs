...
ensureColumn('inventory_lines','stock_unit','TEXT NULL');
...
export function addInventoryLine({sessionId,user,product}){
 ...
 const id=uid('invl');
 const stockUnit=String(product.inventoryUnit||'').trim()||null;
 db.prepare(`INSERT INTO inventory_lines(id,session_id,ean,product_number,product_name,category,theoretical_qty,stock_unit) VALUES(?,?,?,?,?,?,?,?)`).run(id,sessionId,product.ean,product.productNumber||null,product.name,product.category||null,stock,stockUnit);
 audit({storeId:session.store_id,userId:user.id,action:'INVENTORY_LINE_ADDED',entityType:'INVENTORY_LINE',entityId:id,details:{sessionId,ean:product.ean,theoreticalQty:stock,stockUnit}});
