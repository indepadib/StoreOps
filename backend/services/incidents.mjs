...
ensureColumn('incident_evidence','file_bytes','BLOB NULL');
...
export function addEvidence({incidentId,user,dataUrl,fileName='preuve',caption=''}){const incident=db.prepare(`SELECT * FROM incidents WHERE id=?`).get(incidentId);if(!incident)throw Object.assign(new Error('Incident introuvable.'),{status:404});const parsed=parseDataUrl(dataUrl),id=uid('ev'),storageKey=`${id}${parsed.ext}`;try{writeFileSync(join(MEDIA_DIR,storageKey),parsed.buf)}catch{}db.prepare(`INSERT INTO incident_evidence(id,incident_id,kind,file_name,mime_type,storage_key,caption,created_by,file_bytes) VALUES(?,?,?,?,?,?,?,?,?)`).run(id,incidentId,'PHOTO',fileName||storageKey,parsed.mime,storageKey,caption||null,user.id,parsed.buf);audit({storeId:incident.store_id,userId:user.id,action:'INCIDENT_EVIDENCE_ADDED',entityType:'INCIDENT',entityId:incidentId,details:{evidenceId:id,fileName,caption,persisted:true}});return incidentById(incidentId)}
export function mediaById(id){const e=db.prepare(`SELECT e.*,i.store_id FROM incident_evidence e JOIN incidents i ON i.id=e.incident_id WHERE e.id=?`).get(id);if(!e)return null;if(e.file_bytes)return{...e,bytes:Buffer.from(e.file_bytes)};const path=join(MEDIA_DIR,e.storage_key);if(!existsSync(path))return null;const bytes=readFileSync(path);try{db.prepare(`UPDATE incident_evidence SET file_bytes=? WHERE id=? AND file_bytes IS NULL`).run(bytes,id)}catch{}return{...e,bytes}}
