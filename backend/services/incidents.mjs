...
function hydrateIncident(row){
  ...
  const evidence=db.prepare(`SELECT e.*,u.name created_by_name FROM incident_evidence e LEFT JOIN users u ON u.id=e.created_by WHERE e.incident_id=? ORDER BY e.created_at DESC`).all(row.id).map(e=>({...e,content_blob:undefined,available:!!e.content_blob||existsSync(join(MEDIA_DIR,e.storage_key)),url:`/api/media/${e.id}`}));
...
export function addEvidence({incidentId,user,dataUrl,fileName='preuve',caption=''}){const incident=db.prepare(`SELECT * FROM incidents WHERE id=?`).get(incidentId);if(!incident)throw Object.assign(new Error('Incident introuvable.'),{status:404});const parsed=parseDataUrl(dataUrl),id=uid('ev'),storageKey=`${id}${parsed.ext}`;try{writeFileSync(join(MEDIA_DIR,storageKey),parsed.buf)}catch{}db.prepare(`INSERT INTO incident_evidence(id,incident_id,kind,file_name,mime_type,storage_key,caption,created_by,content_blob) VALUES(?,?,?,?,?,?,?,?,?)`).run(id,incidentId,'PHOTO',fileName||storageKey,parsed.mime,storageKey,caption||null,user.id,parsed.buf);audit({storeId:incident.store_id,userId:user.id,action:'INCIDENT_EVIDENCE_ADDED',entityType:'INCIDENT',entityId:incidentId,details:{evidenceId:id,fileName,caption,storage:'SQLITE_BLOB'}});return incidentById(incidentId)}
export function mediaById(id){const e=db.prepare(`SELECT e.*,i.store_id FROM incident_evidence e JOIN incidents i ON i.id=e.incident_id WHERE e.id=?`).get(id);if(!e)return null;if(e.content_blob)return{...e,content_blob:undefined,bytes:Buffer.from(e.content_blob)};const path=join(MEDIA_DIR,e.storage_key);if(!existsSync(path))return null;return{...e,content_blob:undefined,bytes:readFileSync(path)}}
