...
function materializeCommercialDeltas(storeId,businessDate,changes=[]){
 ...
 for(const original of Array.isArray(changes)?changes:[]){
  ...
  const from=c.validFrom||c.effectiveFrom||null,distance=dayDistance(from,businessDate),recentFirstSeen=!previous&&!historical&&distance!==null&&distance>=0&&distance<=7;
  let actionDate=null;
  if(c.actionType==='VERIFY'&&(changed||changedFromHistory||recentFirstSeen)){
    c={...c,actionType:'PROMO_START',signageAction:'INSTALL',priority:c.priority==='CRITICAL'?'CRITICAL':'HIGH',promoLabel:[changed||changedFromHistory?'Promotion modifiée dans Dynamics':'Promotion récente détectée',c.promoLabel].filter(Boolean).join(' · ')};
...
export function syncCommercialControls({storeId,businessDate=todayISO(),changes=[],preserveExisting=false}){
 ...
 for(const c of actionable){
  ...
  if(!Number(info.changes||0)){const required=c.priceControlRequired?1:0;db.prepare(`UPDATE commercial_controls SET status=CASE WHEN price_control_required=0 AND ?=1 THEN 'PENDING' ELSE status END,old_price=?,expected_price=?,promo_label=?,signage_action=?,priority=?,blocking_opening=?,price_control_required=?,price_resolution_source=?,price_resolution_note=? WHERE store_id=? AND business_date=? AND source_key=?`).run(required,c.oldPrice??null,c.expectedPrice??null,c.promoLabel||null,c.signageAction||'VERIFY',priority,blocking,required,c.priceResolutionSource||null,c.priceResolutionNote||null,storeId,businessDate,String(c.sourceKey))}
