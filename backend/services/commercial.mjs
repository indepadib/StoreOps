...
function materializeCommercialDeltas(storeId,businessDate,changes=[]){
 ...
 for(const original of Array.isArray(changes)?changes:[]){
  ...
  const productPrevious=c.productNumber?productHistory.get(storeId,c.productNumber,businessDate):null,historicalFingerprint=historical?fingerprintFor({productNumber:historical.product_number,ean:historical.ean,expectedPrice:historical.expected_price,oldPrice:historical.old_price,promoLabel:historical.promo_label,signageAction:historical.signage_action}):null,changedFromHistory=!!historical&&historicalFingerprint!==fingerprint;
  ...
  if(c.actionType==='VERIFY'&&(changed||changedFromHistory||deltaFirstSeen)){
    const deltaActionType=c.deltaActionType||'PROMO_START',priceDelta=deltaActionType==='PRICE_CHANGE',priorExpected=productPrevious?.expected_price==null?null:Number(productPrevious.expected_price),oldPrice=priceDelta&&c.oldPrice==null&&Number.isFinite(priorExpected)&&Math.abs(priorExpected-Number(c.expectedPrice))>.004?priorExpected:c.oldPrice;
