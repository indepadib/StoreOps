...
export function syncCommercialControls({storeId,businessDate=todayISO(),changes=[],preserveExisting=false}){
 ...
 for(const c of actionable){
  ...
  if(!Number(info.changes||0))db.prepare(`UPDATE commercial_controls SET old_price=?,expected_price=?,promo_label=?,signage_action=?,priority=?,blocking_opening=?,price_control_required=?,price_resolution_source=?,price_resolution_note=? WHERE store_id=? AND business_date=? AND source_key=? AND status='PENDING'`).run(c.oldPrice??null,c.expectedPrice??null,c.promoLabel||null,c.signageAction||'VERIFY',priority,blocking,c.priceControlRequired?1:0,c.priceResolutionSource||null,c.priceResolutionNote||null,storeId,businessDate,String(c.sourceKey));
