...
export function buildInventoryExcel({sessionId,user}){
 ...
 const fmtMap=m=>Object.entries(m||{}).map(([u,v])=>`${v} ${u}`).join(' · ')||'0';
 const workbook=excelWorkbook({sheets:[
  {name:'Synthese',headers:['Indicateur','Valeur'],widths:[220,220],rows:[
   ...
   [txt('Lignes avec écart'),num(inv.metrics?.varianceLines||0)],[txt('Écarts absolus par unité de stock'),txt(fmtMap(inv.metrics?.varianceByUnit))],
   [txt('Écarts absolus en unité de contrôle'),txt(fmtMap(inv.metrics?.controlVarianceByUnit))],
   [txt('Règle seuils'),txt('Seuils évalués en pièce pour les unités, kg pour g/kg, L pour mL/L. Les quantités restent exportées dans leur unité de stock D365.')]
  ]},
  {name:'Ajustements',headers:['Date','Magasin','Session','EAN','Code article','Libellé','Catégorie','Stock théorique','Stock compté final','Ajustement à saisir','Unité stock','Écart contrôle','Unité contrôle','Motif','Commentaire'],widths:[85,100,135,115,105,210,120,95,105,105,80,90,90,160,240],rows:adjustments.map(x=>[
   txt(inv.business_date),txt(inv.store_id),txt(inv.id),txt(x.ean),txt(x.product_number),txt(x.product_name),txt(x.category),num(x.theoretical_qty),num(x.final_qty),num(x.final_variance),txt(x.unit),num(x.variance_control_qty),txt(x.variance_control_unit),txt(reason(INVENTORY_REASON_CODES,x.reason_code)),txt(x.note)
  ])},
  {name:'Comptage complet',headers:['Date','Magasin','Session','EAN','Code article','Libellé','Catégorie','Stock théorique','Unité stock','1er comptage','2e comptage','Stock final','Écart final','Écart contrôle','Unité contrôle','Motif','Commentaire','Statut'],widths:[85,100,135,115,105,210,120,95,80,95,95,95,90,90,90,160,240,100],rows:inv.lines.map(x=>[
   txt(inv.business_date),txt(inv.store_id),txt(inv.id),txt(x.ean),txt(x.product_number),txt(x.product_name),txt(x.category),num(x.theoretical_qty),txt(x.unit),num(x.count1_qty),num(x.count2_qty),num(x.final_qty),num(x.final_variance),num(x.variance_control_qty),txt(x.variance_control_unit),txt(reason(INVENTORY_REASON_CODES,x.reason_code)),txt(x.note),txt(x.status)
  ])}
 ]});
 const fileName=`inventaire_${safeFile(inv.store_id)}_${inv.business_date}_${safeFile(inv.id)}.xls`;
 audit({storeId:inv.store_id,businessDate:inv.business_date,userId:user?.id||null,action:'INVENTORY_EXCEL_EXPORTED',entityType:'INVENTORY_SESSION',entityId:inv.id,details:{rowCount:inv.lines.length,adjustmentLines:adjustments.length,varianceByUnit:inv.metrics?.varianceByUnit||{},controlVarianceByUnit:inv.metrics?.controlVarianceByUnit||{}}});
