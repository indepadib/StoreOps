...
export function buildInventoryExcel({sessionId,user}){
 ...
 const varianceRows=Object.entries(inv.metrics?.varianceByUnit||{}).map(([unit,value])=>[txt(`Écart absolu cumulé (${unit})`),num(value)]);
 const workbook=excelWorkbook({sheets:[
  {name:'Synthese',headers:['Indicateur','Valeur'],widths:[210,170],rows:[
   ...
   [txt('Lignes avec écart'),num(inv.metrics?.varianceLines||0)],...varianceRows,
   [txt('Règle unité'),txt('Chaque quantité et ajustement reste exprimé dans son unité de stock D365. Les g, kg, mL, L et pièces ne sont jamais additionnés entre eux.')]
  ]},
