...
export function buildInventoryExcel({sessionId,user}){
 ...
 const workbook=excelWorkbook({sheets:[
  {name:'Synthese',headers:['Indicateur','Valeur'],widths:[210,170],rows:[
   ...
   [txt('Lignes avec écart'),num(inv.metrics?.varianceLines||0)],...Object.entries(inv.metrics?.absoluteVarianceByUnit||{}).map(([unit,value])=>[txt(`Écart absolu · ${unit}`),num(value)])
  ]},
  {name:'Ajustements',headers:['Date','Magasin','Session','EAN','Code article','Libellé','Catégorie','Unité stock','Stock théorique','Stock compté final','Ajustement à saisir','Motif','Commentaire'],widths:[85,100,135,115,105,210,120,85,95,105,105,160,240],rows:adjustments.map(x=>[
   txt(inv.business_date),txt(inv.store_id),txt(inv.id),txt(x.ean),txt(x.product_number),txt(x.product_name),txt(x.category),txt(x.unit),num(x.theoretical_qty),num(x.final_qty),num(x.final_variance),txt(reason(INVENTORY_REASON_CODES,x.reason_code)),txt(x.note)
  ])},
  {name:'Comptage complet',headers:['Date','Magasin','Session','EAN','Code article','Libellé','Catégorie','Unité stock','Stock théorique','1er comptage','2e comptage','Stock final','Écart final','Motif','Commentaire','Statut'],widths:[85,100,135,115,105,210,120,85,95,95,95,95,90,160,240,100],rows:inv.lines.map(x=>[
   txt(inv.business_date),txt(inv.store_id),txt(inv.id),txt(x.ean),txt(x.product_number),txt(x.product_name),txt(x.category),txt(x.unit),num(x.theoretical_qty),num(x.count1_qty),num(x.count2_qty),num(x.final_qty),num(x.final_variance),txt(reason(INVENTORY_REASON_CODES,x.reason_code)),txt(x.note),txt(x.status)
  ])}
 ]});
