...
export async function getProductCost(storeId,productNumber,{businessDate=null}={}){
 ...
 const select=[...new Set([f.item,f.cost,f.validFrom,f.validTo,f.currency,f.warehouse,f.site,f.unit,f.quantity,config.dynamics.dataAreaId?config.dynamics.dataAreaField:''].filter(Boolean))].join(',');
 const rows=(await odataGet(cfg.entity,{filter:filters.join(' and '),select,top:100,extra:config.dynamics.dataAreaId?'cross-company=true':''}))?.value||[];
