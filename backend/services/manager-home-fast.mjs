...
const HOME_CACHE_MS=Math.max(500,Math.min(5000,Number(globalThis.process?.env?.STOREOPS_MANAGER_HOME_CACHE_MS)||2500));
...
function progressFor(day,group){return day?.id?processProgress(day.id,group):zeroProgress()}
function receiptSummary(storeId,businessDate){
 const receipts=db.prepare(`SELECT COUNT(*) total,COALESCE(SUM(CASE WHEN status!='POSTED' THEN 1 ELSE 0 END),0) active,COALESCE(SUM(CASE WHEN status!='POSTED' AND eta<? THEN 1 ELSE 0 END),0) overdue,COALESCE(SUM(CASE WHEN status!='POSTED' AND date(eta)=? THEN 1 ELSE 0 END),0) due_today FROM receipts WHERE store_id=?`).get(businessDate,businessDate,storeId);
 ...
 return{activeReceipts:Number(receipts.active||0),totalReceipts:Number(receipts.total||0),overdue:Number(receipts.overdue||0),dueToday:Number(receipts.due_today||0),pendingLines:Number(lines.pending||0)}
}
...
function maintenanceSummary(storeId){
 const row=db.prepare(`SELECT COUNT(*) open_count,COALESCE(SUM(CASE WHEN criticality='CRITICAL' THEN 1 ELSE 0 END),0) critical,COALESCE(SUM(CASE WHEN blocking_level IS NOT NULL AND blocking_level!='NONE' THEN 1 ELSE 0 END),0) blocking,COALESCE(SUM(CASE WHEN due_at IS NOT NULL AND datetime(due_at)<CURRENT_TIMESTAMP THEN 1 ELSE 0 END),0) overdue FROM incidents WHERE store_id=? AND status='OPEN' AND (UPPER(COALESCE(category,''))='MAINTENANCE' OR UPPER(COALESCE(source_type,'')) LIKE '%MAINT%')`).get(storeId);
 return{openCount:Number(row.open_count||0),critical:Number(row.critical||0),blocking:Number(row.blocking||0),overdue:Number(row.overdue||0)}
}
function computeManagerHomeFast(storeId,businessDate){
 const day=readStoreDay(storeId,businessDate),opening=progressFor(day,'opening'),closing=progressFor(day,'closing'),commercial=commercialSummary(storeId,businessDate),cash=cashClosingSummary(storeId,businessDate),dlc=dlcSummary(storeId),handover=handoverStats(storeId,businessDate),inventory=inventorySummary(storeId),incidents=incidentStats(storeId),staff=staffingSummary(storeId,businessDate),cold=coldChainSummary(storeId,businessDate),cashOpen=cashOpeningSummary(storeId,businessDate),loss=lossSummary(storeId,businessDate),receipts=receiptSummary(storeId,businessDate),quality=qualitySummary(storeId,businessDate),maintenance=maintenanceSummary(storeId),cycle=dayCycleMetrics(day);
