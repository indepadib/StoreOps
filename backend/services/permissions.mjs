...
export function isControlling(user){return !!user&&user.permissions_profile==='controlling'}
export function isExecutive(user){return !!user&&user.permissions_profile==='executive'}
export function isNetworkReadOnly(user){return isControlling(user)||isExecutive(user)}
...
export function canAccessStore(user,storeId){
 ...
 if(user.role==='ops_director'||isQualityAudit(user)||isNetworkReadOnly(user))return true;
