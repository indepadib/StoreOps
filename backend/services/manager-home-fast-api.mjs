...
export async function handleManagerHomeFastApi({req,url,user}){
 if(req.method!=='GET')return null;
 const p=route(url.pathname,'/api/stores/:storeId/manager-home-fast');if(!p)return null;
 if(!canAccessStore(user,p.storeId))throw Object.assign(new Error('Accès interdit à ce magasin.'),{status:403});
 return{status:200,data:getManagerHomeFast(p.storeId,url.searchParams.get('date')||undefined)}
}
