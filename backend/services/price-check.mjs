...
import { getStoreProductByReference } from './dynamics-stock.mjs';
...
export async function buildPriceCheckContext({storeId,ean,identifier,businessDate=todayISO()}){
 ...
 const product=await getStoreProductByReference(storeId,code);if(!product)throw Object.assign(new Error('Article introuvable Dynamics.'),{status:404,code:'ITEM_NOT_FOUND'});
 ...
 return{
  storeId,businessDate,ean:product.ean||`ITEM:${product.productNumber||code}`,lookupIdentifier:code,priceGroup:pricing?.priceGroup||priceGroup,priceGroups:pricing?.priceGroups||[priceGroup],priceGroupContext:pricing?.priceGroupContext||null,
