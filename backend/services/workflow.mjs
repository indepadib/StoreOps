...
export function validateProcess({storeDay,user,group}){
  ...
  if(p.blockers>0||p.done<p.total||openCritical>0||handoverBlocking>0||commercialBlocking>0||cashOpeningBlocking>0||!handoverReviewed||dlcBlocking>0||cashBlocking>0||lossBlocking>0){
    const reason=!handoverReviewed?'La passation de fin de journée doit être revue avant fermeture.':handoverBlocking?`${handoverBlocking} passation(s) bloquante(s) doivent être résolues avant ouverture.`:commercialBlocking?`${commercialBlocking} changement(s) prix/promo restent à vérifier avant ouverture.`:cashOpeningBlocking?`${cashOpeningBlocking} caisse(s) ne sont pas prêtes. Affectation, fond, POS, TPE, imprimante et shift Dynamics doivent être conformes avant ouverture.`:cashBlocking?'La clôture caisses doit être rapprochée et validée avant fermeture magasin.':lossBlocking?`${lossBlocking} perte(s) / démarque(s) restent à documenter ou poster avant fermeture.`:dlcBlocking?`${dlcBlocking} lot(s) DLC/DDM périmé(s) ou critique(s) restent à traiter avant fermeture.`:'Tous les contrôles obligatoires ne sont pas conformes.';
    const err=new Error(reason);err.status=409;err.details={...p,openCritical,handoverBlocking,commercialBlocking,cashOpeningBlocking,cashOpeningStatus:cashOpen.status,handoverReviewed,dlcBlocking,cashBlocking,cashStatus:cash.status,lossBlocking};throw err;
  }
  if(group==='opening'){
