...
export function validateProcess({storeDay,user,group}){
  ...
  const staffing=group==='opening'?staffingSummary(storeDay.store_id,storeDay.business_date):{blocking:0,status:'NA'};
  const staffingBlocking=group==='opening'?Number(staffing.blocking||0):0;
  ...
  if(group==='opening'){
    ...
    markStaffingOpened({storeId:storeDay.store_id,businessDate:storeDay.business_date,user});
