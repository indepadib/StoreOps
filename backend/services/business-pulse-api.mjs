...
import { managerTodaySnapshot,managerTodaySignals } from './manager-today.mjs';
...
export async function handleBusinessPulseApi({req,url,user}){
 let p=route(url.pathname,'/api/stores/:storeId/manager-today');
 if(p&&req.method==='GET'){
  if(!canAccessStore(user,p.storeId))return forbidden();
  const businessDate=url.searchParams.get('date')||todayISO();
  return{status:200,data:managerTodaySnapshot(p.storeId,businessDate)};
 }
 p=route(url.pathname,'/api/stores/:storeId/manager-today/signals');
 if(p&&req.method==='GET'){
  if(!canAccessStore(user,p.storeId))return forbidden();
  const businessDate=url.searchParams.get('date')||todayISO();
  return{status:200,data:await managerTodaySignals(p.storeId,businessDate)};
 }
