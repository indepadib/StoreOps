...
function hydrate(row){
 ...
 const latest=treatments[0]?.action_type||null;
 const satisfied={
  ...
  CRITICAL:['SHORT_DATE','ISOLATED','DISPOSED'].includes(row.operational_state)&&['MARKDOWN','SHORT_DATE_PROMO','WITHDRAW_SHELF','QUALITY_REVIEW','DESTROY','RETURN_SUPPLIER','DONATE'].includes(latest),
  ALERT:['SHORT_DATE','FEFO','ISOLATED','DISPOSED'].includes(row.operational_state)&&['MARKDOWN','SHORT_DATE_PROMO','FEFO_ROTATE','WITHDRAW_SHELF','RETURN_SUPPLIER'].includes(latest),
