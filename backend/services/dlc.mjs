...
export function evaluateDlc(record){
 ...
 return{stage,daysRemaining:d,threshold:t,...map[stage]};
}
...
export function blockingDlcCount(storeId){return listDlc(storeId,'ACTIVE').filter(x=>x.remaining_quantity>0&&['EXPIRED','DDM_PASSED','CRITICAL'].includes(x.risk.stage)).length}
...
export function addDlcTreatment({id,user,actionType,quantity=0,note='',dataUrl=null,fileName=null,caption=''}){const row=db.prepare(`SELECT * FROM dlc_records WHERE id=?`).get(id);if(!row)throw Object.assign(new Error('Lot DLC introuvable.'),{status:404});if(row.status!=='ACTIVE')throw Object.assign(new Error('Ce lot est déjà clôturé.'),{status:409});const action=DLC_ACTIONS.find(x=>x.code===actionType);if(!action)throw Object.assign(new Error('Action DLC inconnue.'),{status:400});const before=Number(row.remaining_quantity??row.quantity),q=Number(quantity||0);if(action.removesStock&&(!(q>0)||q>before))throw Object.assign(new Error('La quantité traitée doit être > 0 et ≤ à la quantité restante.'),{status:400});if(action.proof&&!dataUrl)throw Object.assign(new Error('Une preuve photo/PDF est obligatoire pour cette action.'),{status:409});const after=action.removesStock?Math.max(0,before-q):before;const operational={WITHDRAW_SHELF:'ISOLATED',QUALITY_REVIEW:'ISOLATED',MARKDOWN:'SHORT_DATE',SHORT_DATE_PROMO:'SHORT_DATE',FEFO_ROTATE:'FEFO',NO_ACTION:'SELLABLE',DESTROY:'DISPOSED',RETURN_SUPPLIER:'DISPOSED',DONATE:'DISPOSED'}[actionType]||row.operational_state;const tid=uid('dlct');db.prepare(`INSERT INTO dlc_treatments(id,dlc_id,action_type,quantity,quantity_before,quantity_after,note,performed_by) VALUES(?,?,?,?,?,?,?,?)`).run(tid,id,actionType,action.removesStock?q:0,before,after,note||null,user.id);if(dataUrl)addEvidence({dlcId:id,treatmentId:tid,user,dataUrl,fileName,caption});const status=after<=0?'CLOSED':'ACTIVE';db.prepare(`UPDATE dlc_records SET remaining_quantity=?,operational_state=?,status=?,closed_by=CASE WHEN ?='CLOSED' THEN ? ELSE closed_by END,closed_at=CASE WHEN ?='CLOSED' THEN CURRENT_TIMESTAMP ELSE closed_at END,last_control_at=CURRENT_TIMESTAMP WHERE id=?`).run(after,operational,status,status,user.id,status,id);const refreshed=db.prepare(`SELECT * FROM dlc_records WHERE id=?`).get(id),risk=evaluateDlc(refreshed);if(status==='ACTIVE')db.prepare(`UPDATE dlc_records SET next_control_at=? WHERE id=?`).run(nextControl(risk.stage),id);audit({storeId:row.store_id,userId:user.id,action:'DLC_TREATMENT',entityType:'DLC',entityId:id,details:{actionType,quantity:q,before,after,note}});return hydrate(db.prepare(`SELECT * FROM dlc_records WHERE id=?`).get(id))}
