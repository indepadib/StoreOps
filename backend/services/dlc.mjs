...
ensureColumn('dlc_records','retail_price_unit','TEXT NULL');
ensureColumn('dlc_records','retail_price_quantity','REAL NULL');
ensureColumn('dlc_records','unit_cost_value','REAL NULL');
ensureColumn('dlc_records','cost_unit','TEXT NULL');
ensureColumn('dlc_records','cost_basis_quantity','REAL NULL');
ensureColumn('dlc_records','cost_source','TEXT NULL');
ensureColumn('dlc_records','cost_state','TEXT NULL');
...
export function createDlcRecord({storeId,user,product,expiryDate,quantity,zone='Rayon',lotRef=null,comment='',expiryType='DLC',department=null,family=null,unit='pièce',sourceType='MANUAL'}){
 ...
 db.prepare(`UPDATE dlc_records SET product_number=?,category=?,unit_retail_value=?,retail_price_unit=?,retail_price_quantity=?,unit_cost_value=?,cost_unit=?,cost_basis_quantity=?,cost_source=?,cost_state=? WHERE id=?`).run(product.productNumber||null,product.category||null,product.price==null?null:Number(product.price),product.retailUnit||product.unit||null,Number(product.retailPriceQuantity||1),product.unitCost==null?null:Number(product.unitCost),product.costUnit||null,Number(product.costBasisQuantity||1),product.costSource||null,product.costState||'UNAVAILABLE',id);
...
export function addDlcTreatment({id,user,actionType,quantity=0,note='',dataUrl=null,fileName=null,caption=''}){
 ...
 if(lossReason&&q>0){generatedLoss=createLossRecord({storeId:row.store_id,businessDate:todayISO(),user,product:{ean:row.ean,name:row.product_name,productNumber:row.product_number||null,category:row.category||null,price:row.unit_retail_value,retailUnit:row.retail_price_unit||row.unit||null,retailPriceQuantity:Number(row.retail_price_quantity||1),unitCost:row.unit_cost_value,costUnit:row.cost_unit||null,costBasisQuantity:Number(row.cost_basis_quantity||1),costSource:row.cost_source||null,costState:row.cost_state||'UNAVAILABLE'},reasonCode:lossReason,quantity:q,unit:row.unit||'pièce',note:[`Générée automatiquement depuis DLC ${row.expiry_date}`,row.lot_ref?`lot ${row.lot_ref}`:null,note||null].filter(Boolean).join(' · '),sourceType:'DLC_TREATMENT',sourceId:tid,evidenceAlreadySatisfied:!!evidenceId,evidenceSourceType:evidenceId?'DLC_TREATMENT':null,evidenceSourceId:evidenceId?tid:null})}
