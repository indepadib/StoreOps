...
ensureColumn('dlc_evidence','content_blob','BLOB NULL');
...
function hydrate(row){
 ...
 const evidence=db.prepare(`SELECT e.*,u.name created_by_name FROM dlc_evidence e LEFT JOIN users u ON u.id=e.created_by WHERE e.dlc_id=? ORDER BY e.created_at DESC`).all(row.id).map(e=>({...e,content_blob:undefined,available:!!e.content_blob||existsSync(join(MEDIA_DIR,e.storage_key)),url:`/api/dlc-media/${e.id}`}));
...
function addEvidence({dlcId,treatmentId,user,dataUrl,fileName='preuve',caption=''}){const p=parseDataUrl(dataUrl),id=uid('dlcev'),storageKey=`${id}${p.ext}`;try{writeFileSync(join(MEDIA_DIR,storageKey),p.buf)}catch{}db.prepare(`INSERT INTO dlc_evidence(id,dlc_id,treatment_id,file_name,mime_type,storage_key,caption,created_by,content_blob) VALUES(?,?,?,?,?,?,?,?,?)`).run(id,dlcId,treatmentId,fileName||storageKey,p.mime,storageKey,caption||null,user.id,p.buf);return id}
export function dlcMedia(id){const e=db.prepare(`SELECT e.*,d.store_id FROM dlc_evidence e JOIN dlc_records d ON d.id=e.dlc_id WHERE e.id=?`).get(id);if(!e)return null;if(e.content_blob)return{...e,content_blob:undefined,bytes:Buffer.from(e.content_blob)};const path=join(MEDIA_DIR,e.storage_key);if(!existsSync(path))return null;return{...e,content_blob:undefined,bytes:readFileSync(path)}}
