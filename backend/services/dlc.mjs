...
ensureColumn('dlc_evidence','file_bytes','BLOB NULL');
function ensureColumn(table,column,definition){const cols=db.prepare(`PRAGMA table_info(${table})`).all();if(!cols.some(c=>c.name===column))db.exec(`ALTER TABLE ${table} ADD COLUMN ${column} ${definition}`)}
...
function addEvidence({dlcId,treatmentId,user,dataUrl,fileName='preuve',caption=''}){const p=parseDataUrl(dataUrl),id=uid('dlcev'),storageKey=`${id}${p.ext}`;try{writeFileSync(join(MEDIA_DIR,storageKey),p.buf)}catch{}db.prepare(`INSERT INTO dlc_evidence(id,dlc_id,treatment_id,file_name,mime_type,storage_key,caption,created_by,file_bytes) VALUES(?,?,?,?,?,?,?,?,?)`).run(id,dlcId,treatmentId,fileName||storageKey,p.mime,storageKey,caption||null,user.id,p.buf);return id}
export function dlcMedia(id){const e=db.prepare(`SELECT e.*,d.store_id FROM dlc_evidence e JOIN dlc_records d ON d.id=e.dlc_id WHERE e.id=?`).get(id);if(!e)return null;if(e.file_bytes)return{...e,bytes:Buffer.from(e.file_bytes)};const path=join(MEDIA_DIR,e.storage_key);if(!existsSync(path))return null;const bytes=readFileSync(path);try{db.prepare(`UPDATE dlc_evidence SET file_bytes=? WHERE id=? AND file_bytes IS NULL`).run(bytes,id)}catch{}return{...e,bytes}}
