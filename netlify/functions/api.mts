...
function bridgeBackendEnvironment(){
  const keys=[
    ...
    'STOREOPS_STAFFING_SOURCE','STOREOPS_CASH_OPENING_SOURCE','STOREOPS_STOCK_SIGNALS_CACHE_SECONDS','STOREOPS_RECEIVING_HEALTH_MAX_AGE_MINUTES','STOREOPS_STORE_IDENTITY_CACHE_SECONDS',
    'D365_MODE','D365_PRODUCT_READ_MODE','D365_STOCK_READ_MODE','D365_PRICE_READ_MODE','D365_PROMOTION_READ_MODE','D365_RECEIVING_READ_MODE','D365_ASSORTMENT_READ_MODE','D365_TAXONOMY_READ_MODE','D365_STORE_ENTITY',
...
function statelessHealth(request:Request){
  ...
  const response=Response.json({ok:true,service:'StoreOps API',version:envValue('STOREOPS_VERSION')||'2.18.0',authMode:envValue('AUTH_MODE')||'entra',dynamicsMode:envValue('D365_MODE')||'simulated',configurationIssues:[],diagnostics:{source:'NETLIFY_STATELESS_HEALTH'}});
