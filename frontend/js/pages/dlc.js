...
function historyPanel(r){
 ...
 return`<details class="dlc-history"><summary>Historique (${r.treatments?.length||0} action(s), ${r.evidence?.length||0} preuve(s))</summary>
   ...
   <div class="evidence-grid" style="margin-top:8px">${(r.evidence||[]).map(e=>e.available===false?`<div class="evidence-card unavailable"><strong>${esc(e.file_name)}</strong><span>Ancienne preuve indisponible après migration du stockage</span><small>${esc(e.created_by_name||'')} · ${dt(e.created_at)}</small></div>`:`<button class="evidence-card" data-dlc-evidence="${e.id}"><strong>${esc(e.file_name)}</strong><span>${esc(e.caption||'Preuve DLC')}</span><small>${esc(e.created_by_name||'')} · ${dt(e.created_at)}</small></button>`).join('')}</div>
 </details>`}
