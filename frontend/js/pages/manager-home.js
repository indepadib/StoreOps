...
export async function renderManagerHome(){
  ...
  $('#todayContent').innerHTML=`
    <div class="manager-home">
      ...
      ${phaseStrip(phase)}
      <section class="manager-next ${next.level==='CRITICAL'?'critical':''}"><div class="manager-next-copy"><span class="manager-eyebrow">À faire maintenant</span><h2>${esc(next.title)}</h2><p>${esc(next.detail)}</p></div><button class="btn brand manager-main-cta" data-manager-go="${esc(next.page)}">${esc(next.cta)} <span>→</span></button></section>
      ${complianceCard(compliance)}
      <div class="manager-quick-grid" style="margin-top:16px">
        <button class="manager-quick" data-manager-go="managerJourney"><span>Fait dans StoreOps</span><strong>${compliance.percent}%</strong><small>${compliance.done}/${compliance.total} obligation(s) applicable(s)</small></button>
      ...
      <section class="card manager-day-progress"><div class="row"><div><strong>Progression du parcours</strong><div class="small muted">La complétude ci-dessus confirme ce qui a été réellement tracé ; ici vous voyez l’avancement ouverture / fermeture.</div></div>${status(managerPhaseLabel(phase),phase==='CLOSED'?'ok':phase==='OPENING'||phase==='CLOSING'?'warn':'neutral')}</div><div class="manager-progress-row"><span>Ouverture</span>${progress(d.opening?.percent||0)}<strong>${d.opening?.percent||0}%</strong></div><div class="manager-progress-row"><span>Fermeture</span>${progress(d.closing?.percent||0)}<strong>${d.closing?.percent||0}%</strong></div></section>
    </div>`;
}
