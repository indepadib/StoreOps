...
export async function renderToday(){
  ...
  <div class="grid g2">
    ...
    <div class="grid g2"><div class="card"><div class="label">DLC à risque</div><div class="kpi">${d.dlcAtRisk}</div><div class="small muted">${d.dlc?`${d.dlc.expired||0} périmée(s) · ${d.dlc.critical||0} critique(s) · ${d.dlc.pendingActions||0} action(s)`:``}</div></div><div class="card"><div class="label">Incidents</div><div class="kpi">${d.incidents}</div><div class="small muted">${d.criticalIncidents} critique(s) · ${d.overdueIncidents||0} en retard</div></div><div class="card"><div class="label">Passations</div><div class="kpi">${d.handover?.pending||0}</div><div class="small muted">${d.handover?.blocking||0} bloquante(s) · ${d.handover?.unacknowledged||0} non lue(s)</div></div><div class="card"><div class="label">Contrôles qualité</div><div class="kpi">${d.qualityControls}</div></div><div class="card"><div class="label">Qté refusée</div><div class="kpi">${d.qualityRejected}</div></div></div>
  </div>
