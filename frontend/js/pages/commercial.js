...
const priceSourceLabel=v=>({BASE_PRICE:'Prix fiche',TRADE_AGREEMENT:'Accord tarifaire',PROMOTION:'Promotion'}[v]||'Dynamics');
...
function priceCheckResult(c){
 ...
 const agreement=c.tradeAgreement?`<div><span>Accord tarifaire actif</span><strong>${money(c.tradeAgreement.unitPrice)}</strong><small>${esc(c.tradeAgreement.priceGroup||'Retail')}${c.tradeAgreement.validTo?` · fin ${esc(String(c.tradeAgreement.validTo))}`:''}</small></div>`:'';
 return`<div class="card price-check-detail"><div class="price-check-product-head"><div><div class="small muted">Code article <strong>${esc(c.product.productNumber||'—')}</strong> · ${esc(c.product.category||'')}</div><h3>${esc(c.product.name)}</h3><div class="small muted">EAN ${esc(c.product.ean||c.ean||'—')} · Groupes prix ${esc((c.priceGroups||[c.priceGroup]).join(' · '))}</div></div><div class="price-check-price-badge">${status(money(c.expectedUnitPrice),'ok')}</div></div><div class="commercial-grid"><div><span>Prix fiche</span><strong>${money(c.basePrice?.price)}</strong></div>${agreement}<div><span>Prix attendu</span><strong>${money(c.expectedUnitPrice)}</strong><small>${esc(priceSourceLabel(c.effectivePriceSource))}</small></div><div><span>Stock</span><strong>${c.product.stock??'—'}</strong></div><div><span>Disponible</span><strong>${c.product.availableStock??'—'}</strong></div></div>${promo}${scanInfo}${correction}<div class="form-grid" style="margin-top:10px"><div class="field"><label>Prix constaté rayon *</label><input id="priceCheckObserved" type="number" min="0" step="0.01" placeholder="${c.expectedUnitPrice??''}"></div><div class="field"><label>Signalétique conforme *</label><select id="priceCheckSignage"><option value="">— Choisir —</option><option value="true">Oui</option><option value="false">Non</option></select></div><div class="field"><label>Exécution rayon conforme *</label><select id="priceCheckExecution"><option value="">— Choisir —</option><option value="true">Oui</option><option value="false">Non</option></select></div>${evidence}</div><button class="btn brand price-check-submit" id="priceCheckSubmit">${c.openIncident?'Valider la correction et clôturer':'Valider le contrôle'}</button><div class="small muted price-check-help">${c.openIncident?'La clôture n’est possible que si le nouveau scan est conforme et la preuve photo enregistrée.':'En cas d’écart, StoreOps crée automatiquement un incident avec preuve obligatoire.'}</div></div>`;
}
...
async function lookupPrice(){try{const ean=$('#priceCheckEan')?.value.trim();if(!ean)throw new Error('Scanne ou saisis un EAN, un code article ou un code HS.');scanCtx=await api(`/api/stores/${app.storeId}/price-check/context/${encodeURIComponent(ean)}`);$('#priceCheckResult').innerHTML=priceCheckResult(scanCtx);$('#priceCheckSubmit')?.addEventListener('click',submitPriceCheck)}catch(e){toast(e.message)}}
...
async function submitPriceCheck(){
 ...
 try{
  ...
  const result=await api(`/api/stores/${app.storeId}/price-check`,{method:'POST',body:JSON.stringify({identifier:scanCtx.lookupIdentifier||scanCtx.product?.productNumber||scanCtx.ean,ean:scanCtx.ean,observedPrice:Number(observed),signageOk:sg==='true',executionOk:ex==='true',tolerance:Number(cfg.policy.price_tolerance||0.01),evidenceDataUrl,evidenceFileName:file?.name||null,evidenceCaption:'Preuve après correction prix/promo'})});
