...
function controlCard(c){
 ...
 const incidentOpen=c.incident&&c.incident.status==='OPEN',priceBlocked=!!c.price_control_required&&c.expected_price==null;
 return`<article class="card commercial-card ${c.status==='MISMATCH'?'commercial-mismatch':''}"><div class="row"><div><div class="small muted">${esc(actionLabel(c.action_type))} · ${esc(c.category||'')}</div><h3>${esc(c.product_name)}</h3><div class="small muted">${esc(c.ean)}${c.product_number?' · '+esc(c.product_number):''}</div></div><div class="row">${status(c.priority,priKind(c.priority))}${status(priceBlocked?'Prix à résoudre':stateLabel(c.status),priceBlocked?'danger':stateKind(c.status))}</div></div><div class="commercial-grid"><div><span>Ancien prix</span><strong>${money(c.old_price)}</strong></div><div><span>Prix attendu</span><strong>${priceBlocked?'Indisponible':money(c.expected_price)}</strong></div><div><span>Signalétique</span><strong>${esc(signageLabel(c.signage_action))}</strong></div><div><span>Action rayon</span><strong>${esc(expectedAction)}</strong></div></div>${c.promo_label?`<div class="commercial-promo"><span>Information promo</span><strong>${esc(c.promo_label)}</strong></div>`:''}${priceBlocked?`<div class="banner ban-danger"><strong>Contrôle prix bloqué</strong><div class="small" style="margin-top:4px">StoreOps a détecté la promotion mais n’a pas encore résolu son prix attendu. Rafraîchir Dynamics ou vérifier le mapping prix avant validation. Aucun contrôle ne sera marqué conforme sans prix.</div></div>`:''}${c.issues?.length?`<div class="banner ban-danger"><strong>Écart détecté</strong><div class="small" style="margin-top:4px">${c.issues.map(esc).join('<br>')}</div></div>`:''}${c.status==='VERIFIED'&&!priceBlocked?`<div class="banner ban-info"><strong>Contrôle terrain conforme.</strong> ${esc(c.controlled_by_name||'')} · ${dt(c.controlled_at)}${incidentOpen?' · L’incident initial reste à clôturer avec sa preuve.':''}</div>`:''}${incidentOpen?`<button class="btn soft" data-open-incident="${c.incident.id}" style="margin-top:9px">Ouvrir l’incident lié</button>`:''}${canManage()?controlForm(c):''}</article>`}
function controlForm(c){const blocked=!!c.price_control_required&&c.expected_price==null;return`<details class="commercial-control" ${c.status!=='VERIFIED'?'open':''}><summary>${c.status==='MISMATCH'?'Corriger et recontrôler':c.status==='VERIFIED'?'Recontrôler':'Effectuer le contrôle'}</summary><div class="form-grid" style="margin-top:9px">${c.expected_price!=null?`<div class="field"><label>Prix constaté en rayon *</label><input data-commercial-price="${c.id}" type="number" min="0" step="0.01" value="${c.observed_price??''}" placeholder="${c.expected_price}"></div>`:''}<div class="field"><label>Signalétique conforme *</label><select data-commercial-signage="${c.id}" ${blocked?'disabled':''}><option value="">— Choisir —</option><option value="true" ${Number(c.signage_ok)===1?'selected':''}>Oui</option><option value="false" ${Number(c.signage_ok)===0?'selected':''}>Non</option></select></div><div class="field"><label>Exécution rayon conforme *</label><select data-commercial-execution="${c.id}" ${blocked?'disabled':''}><option value="">— Choisir —</option><option value="true" ${Number(c.execution_ok)===1?'selected':''}>Oui</option><option value="false" ${Number(c.execution_ok)===0?'selected':''}>Non</option></select></div><div class="field full"><label>Note</label><input data-commercial-note="${c.id}" value="${esc(c.note||'')}" placeholder="Correction, emplacement, remarque…" ${blocked?'disabled':''}></div></div><button class="btn brand" data-commercial-submit="${c.id}" style="margin-top:9px" ${blocked?'disabled':''}>${blocked?'Prix attendu requis':'Valider le contrôle'}</button></details>`}
