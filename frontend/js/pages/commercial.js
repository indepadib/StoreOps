...
function bindCommercial(){
 document.querySelectorAll('[data-commercial-history]').forEach(b=>b.addEventListener('click',()=>loadCommercialHistory(b.dataset.commercialHistory,b.dataset.productNumber,b)));
...
async function loadCommercialHistory(id,productNumber,button){
 const target=document.querySelector(`[data-commercial-history-result="${id}"]`);if(button)button.disabled=true;if(target)target.textContent='Lecture historique…';
 try{
  const h=await api(`/api/stores/${app.storeId}/items/${encodeURIComponent(productNumber)}/price-history?days=90`),last=(h.observedSales?.items||[])[0]||null;
  if(target)target.innerHTML=last?`Dernier prix caisse observé : <strong>${money(last.weightedUnitPrice)}</strong> · ${esc(last.date)}<br><span class="muted">Prix caisse observé ≠ ancien prix de base garanti.</span>`:'Aucun prix caisse historique trouvé.';
 }catch(e){if(target)target.textContent='Historique indisponible : '+e.message}
