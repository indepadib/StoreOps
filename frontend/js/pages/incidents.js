...
function evidenceCard(e){return e.available===false?`<div class="evidence-card unavailable"><strong>${esc(e.file_name)}</strong><span>Ancienne preuve indisponible après migration du stockage</span><small>${esc(e.created_by_name||'—')} · ${dt(e.created_at)}</small></div>`:`<button class="evidence-card" data-view-evidence="${e.id}"><strong>${esc(e.file_name)}</strong><span>${esc(e.caption||'Preuve photo')}</span><small>${esc(e.created_by_name||'—')} · ${dt(e.created_at)}</small></button>`}
