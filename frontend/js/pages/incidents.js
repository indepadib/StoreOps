...
function bindDetailForm(){
  const actionInput=$('#incActionTitle'),actionBtn=$('#addIncidentAction');
  const syncActionButton=()=>{if(actionBtn)actionBtn.disabled=!actionInput?.value.trim()};actionInput?.addEventListener('input',syncActionButton);syncActionButton();
  const resolutionInput=$('#incidentResolutionNote'),resolveBtn=$('#resolveIncident');
  const syncResolveButton=()=>{if(resolveBtn)resolveBtn.disabled=!!activeIncident?.open_actions||!resolutionInput?.value.trim()||(!!activeIncident?.requires_evidence&&!(activeIncident?.evidence||[]).length)};resolutionInput?.addEventListener('input',syncResolveButton);syncResolveButton();
  actionBtn?.addEventListener('click',async()=>{try{const title=actionInput?.value.trim()||'';if(!title)throw new Error('Saisis l’action corrective à réaliser.');await api(`/api/incidents/${activeIncident.id}/actions`,{method:'POST',body:JSON.stringify({title,note:$('#incActionNote').value.trim(),assignedTo:$('#incActionAssignee').value||null,dueAt:$('#incActionDue').value||null})});toast('Action corrective ajoutée.');await openIncident(activeIncident.id);await renderIncidents()}catch(e){toast(e.message)}});
  ...
  resolveBtn?.addEventListener('click',async()=>{try{if(activeIncident?.open_actions)throw new Error(`Termine d’abord ${activeIncident.open_actions} action(s) corrective(s).`);if(activeIncident?.requires_evidence&&!(activeIncident?.evidence||[]).length)throw new Error('Ajoute la preuve photo obligatoire avant clôture.');const resolutionNote=resolutionInput?.value.trim()||'';if(!resolutionNote)throw new Error('Saisis le compte-rendu de résolution.');await api(`/api/incidents/${activeIncident.id}/resolve`,{method:'POST',body:JSON.stringify({resolutionNote})});toast('Incident clôturé.');await openIncident(activeIncident.id);await renderIncidents()}catch(e){toast(e.message)}});
