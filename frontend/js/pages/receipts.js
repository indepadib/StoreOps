...
async function drawReceipts(errors={}){
 ...
 const tabs=`<div class="receipt-doc-tabs">
   <button class="btn ${documentType==='PO'?'brand':'ghost'}" data-receipt-type="PO"><span>PO · Commandes d'achat</span><span class="pill">${docCount('PO')}</span></button>
...
function documentRow(r){
 ...
 const secondary=type==='PO'
  ?`${r.source_vendor_account?`Compte fournisseur ${esc(r.source_vendor_account)} · `:''}Créé le ${dt(created||r.eta)} · Livraison ${dt(r.eta)} · Warehouse destination ${esc(r.source_warehouse_id||'—')}`
  :`Warehouse expéditeur ${esc(origin)} · Destination ${esc(r.source_destination||r.source_warehouse_id||'—')} · Réception prévue ${dt(r.eta)}`;
