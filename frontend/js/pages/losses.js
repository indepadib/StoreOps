...
export async function renderLosses(){
  ...
  $('#lossesContent').innerHTML=`
    ...
    ${s.unvaluedCount?`<div class="banner ban-warn loss-followup-banner"><strong>${s.unvaluedCount} ligne(s) non valorisée(s)</strong><span>Le CostPrice / prix d’achat n’est pas encore disponible pour ces articles. StoreOps ne remplace jamais le coût par le prix de vente.</span>${canManage()?'<button class="btn soft" id="revalueLossesBtn" style="margin-top:8px">Rechercher les coûts D365</button>':''}</div>`:''}${s.blocking?'<div class="banner ban-warn loss-followup-banner"><strong>La saisie est faite, StoreOps garde le suivi.</strong><span>Les preuves / approbations / imports encore nécessaires sont listés dans le suivi ci-dessous.</span></div>':''}
...
function bind(){
 ...
 const revalue=$('#revalueLossesBtn');if(revalue)revalue.onclick=async()=>{try{revalue.disabled=true;revalue.textContent='Recherche CostPrice…';const result=await api(`/api/stores/${app.storeId}/losses/revalue`,{method:'POST',body:JSON.stringify({})});toast(result.updated?String(result.updated)+' ligne(s) revalorisée(s) au coût D365.':'Aucun coût D365 supplémentaire trouvé.');renderLosses()}catch(e){revalue.disabled=false;revalue.textContent='Rechercher les coûts D365';toast(e.message)}};
