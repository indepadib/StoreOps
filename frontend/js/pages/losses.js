...
function createPanel(config){return`<div class="card loss-create" style="margin-top:14px">
  ...
  <div class="form-grid" style="margin-top:12px">
    <div class="field full"><label>Article *</label><div class="loss-lookup-row"><input id="lossEan" inputmode="numeric" autocomplete="off" placeholder="Scanner ou saisir le code-barres"><button class="btn soft" id="lossLookup" type="button">Identifier</button></div><div id="lossProductPreview" class="small muted" style="margin-top:6px">Scanne l’article pour vérifier son identité avant la saisie.</div></div>
    ...
    <div class="field full"><label>Photo de preuve</label><input id="lossEvidence" type="file" accept="image/jpeg,image/png,image/webp" capture="environment"><div class="small muted">Recommandée immédiatement. Elle devient obligatoire si la valeur dépasse le seuil réseau ou si le prix n’est pas disponible.</div></div>
  </div>
...
async function lookupLossProduct(){const input=$('#lossEan'),preview=$('#lossProductPreview'),ean=input?.value.trim();if(!ean)throw new Error('Scanne ou saisis un EAN.');try{if(preview)preview.textContent='Identification…';lossProduct=await api(`/api/stores/${app.storeId}/products/${encodeURIComponent(ean)}`);if(preview)preview.innerHTML=`<strong>${esc(lossProduct.name||lossProduct.productNumber||ean)}</strong> · ${esc(lossProduct.productNumber||'')} ${lossProduct.price!=null?'· '+fmtMoney(lossProduct.price):'· prix indisponible'}`;return lossProduct}catch(e){lossProduct=null;if(preview)preview.textContent=e.message;toast(e.message);throw e}}
function lossFileDataUrl(file){return new Promise((resolve,reject)=>{if(!file)return resolve(null);if(file.size>8*1024*1024)return reject(new Error('Photo trop volumineuse.'));const fallback=()=>{const r=new FileReader();r.onload=()=>resolve(r.result);r.onerror=()=>reject(new Error('Impossible de lire la photo.'));r.readAsDataURL(file)};if(!String(file.type||'').startsWith('image/'))return fallback();const url=URL.createObjectURL(file),img=new Image();img.onload=()=>{try{const max=1600,scale=Math.min(1,max/Math.max(img.naturalWidth||1,img.naturalHeight||1)),canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(img.naturalWidth*scale));canvas.height=Math.max(1,Math.round(img.naturalHeight*scale));canvas.getContext('2d').drawImage(img,0,0,canvas.width,canvas.height);const out=canvas.toDataURL('image/jpeg',.78);URL.revokeObjectURL(url);resolve(out)}catch{URL.revokeObjectURL(url);fallback()}};img.onerror=()=>{URL.revokeObjectURL(url);fallback()};img.src=url})}
...
function bind(){
 $('#lossLookup')?.addEventListener('click',lookupLossProduct);$('#lossEan')?.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();lookupLossProduct()}});
 const create=$('#createLossBtn');if(create)create.onclick=async()=>{if(create.disabled)return;try{const ean=$('#lossEan').value.trim(),quantity=Number($('#lossQty').value);if(!ean)throw new Error('EAN obligatoire.');if(!Number.isFinite(quantity)||quantity<=0)throw new Error('Quantité obligatoire.');if(!lossProduct||String(lossProduct.ean)!==ean)await lookupLossProduct();const file=$('#lossEvidence')?.files?.[0]||null,unitPrice=Number(lossProduct?.price),estimated=Number.isFinite(unitPrice)?unitPrice*quantity:null,requiresPhoto=estimated==null||estimated>=Number(cfg?.policy?.evidence_threshold_dh||0);if(requiresPhoto&&!file)throw new Error('Une photo de preuve est obligatoire pour cette perte.');const evidenceDataUrl=file?await lossFileDataUrl(file):null;create.disabled=true;create.textContent='Enregistrement…';const result=await api(`/api/stores/${app.storeId}/losses`,{method:'POST',body:{ean,reasonCode:$('#lossReason').value,quantity,unit:$('#lossUnit').value,note:$('#lossNote').value.trim(),evidenceDataUrl,evidenceFileName:file?.name||null,evidenceCaption:'Preuve terrain démarque'}});toast(result?.incidentResolved?'Perte enregistrée avec preuve.':'Perte enregistrée.');lossProduct=null;await renderLosses()}catch(e){toast(e.message)}finally{if(create?.isConnected){create.disabled=false;create.textContent='Enregistrer la perte'}}};
