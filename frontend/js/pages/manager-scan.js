...
function priceHistoryHtml(h){
 ...
 if(!hasAny){const activation=h?.activation||{},detail=activation.connected?'La source tarifaire est connectée mais aucun ancien tarif n’a été trouvé pour cet article.':activation.status==='SMOKE_FAILED'?'La source Trade Agreements détectée n’a pas encore passé les contrôles de cohérence D365.':activation.status==='NOT_DISCOVERED'?'Aucune source Trade Agreements datée exploitable n’a encore été identifiée dans D365.':'Le prix actuel reste disponible. StoreOps n’invente pas d’anciennes valeurs lorsqu’aucune source fiable n’est exposée.';return `<section class="manager-price-history"><div class="manager-price-history-head"><div><span class="manager-eyebrow">HISTORIQUE DE PRIX</span><strong>Aucun historique exploitable pour le moment</strong></div></div><p class="manager-price-history-note">${esc(detail)}</p></section>`;}
...
async function loadPriceHistory(btn){
 ...
 try{
  let history=priceHistoryCache.get(`${app.storeId}|${sku}`);if(!history){if(app.showcase)history=showcasePriceHistory(lastContext);else{try{const healed=await api(`/api/stores/${app.storeId}/items/${encodeURIComponent(sku)}/price-history/auto-connect?days=90`,{method:'POST'});history={...(healed?.history||{}),activation:healed?.autoConnect||null}}catch{history=await api(`/api/stores/${app.storeId}/items/${encodeURIComponent(sku)}/price-history?days=90`)}}priceHistoryCache.set(`${app.storeId}|${sku}`,history)}
