...
function priceHistoryHtml(h){
 const agreements=(h?.tradeAgreements?.items||[]).slice(0,10),observed=observedChanges(h?.observedSales?.items||[]),app=h?.tradeAgreements?.applicability||null,hasAny=agreements.length||observed.length;
 ...
 const agreementsHtml=agreements.length?`<div class="manager-price-history-group"><div class="manager-price-history-title"><strong>Tarifs paramétrés</strong><span>${app?.status==='UNIQUE'?'1 accord applicable à la date demandée':app?.status==='AMBIGUOUS'?'Accords actifs ambigus — prix non imposé':'Accords pertinents pour ce magasin'}</span></div>${agreements.map(x=>`<div class="manager-price-row"><div><strong>${money(x.price)}</strong><span>${esc(x.priceGroup||x.customer||x.warehouse||'Tarif')}</span></div><small>${x.from?dateLabel(x.from):'Début non renseigné'}${x.to?` → ${dateLabel(x.to)}`:' → en cours'}</small></div>`).join('')}</div>`:'';
