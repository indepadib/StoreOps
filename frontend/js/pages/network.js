...
export async function renderNetwork(){
  ...
  <div class="grid g4">
    ...
    <div class="card"><div class="label">DLC urgentes</div><div class="kpi">${urgentDlc}</div><div class="small muted">périmées / critiques réseau</div></div><div class="card"><div class="label">Passations bloquantes</div><div class="kpi">${blockingHandover}</div><div class="small muted">à résoudre avant ouverture</div></div>
  </div>
...
const risk=r=>Number(r.sla?.escalated||0)*250+Number(r.handover?.blocking||0)*220+(Number(r.dlc?.expired||0)+Number(r.dlc?.critical||0))*180+Number(r.sla?.overdue||0)*140+Number(r.criticalIncidents||0)*100+Number(r.opening.blockers||0)*30+Number(r.openIncidents||0)*8+(r.day.opening_status==='OPENED'?0:10);
function card(r){
  ...
  return`<article class="card network-store ${danger?'critical':''}">
    ...
    <div class="network-signals"><div><span>Incidents</span><strong>${r.openIncidents}</strong></div><div><span>DLC urgentes</span><strong>${Number(r.dlc?.expired||0)+Number(r.dlc?.critical||0)}</strong></div><div><span>Passations</span><strong>${r.handover?.pending||0}</strong></div><div><span>Bloquantes</span><strong>${r.handover?.blocking||0}</strong></div></div>
