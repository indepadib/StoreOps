...
export async function renderNetwork(){
  ...
  const rows=await Promise.all(base.map(async r=>{try{const [inc,loss,cashOpening]=await Promise.all([api(`/api/stores/${r.id}/incidents?status=OPEN`),api(`/api/stores/${r.id}/losses`),api(`/api/stores/${r.id}/cash-opening`)]);return{...r,sla:inc.stats,loss:loss.summary,cashOpening:cashOpening.summary}}catch{return{...r,sla:{open:r.openIncidents||0,critical:r.criticalIncidents||0,overdue:0,escalated:0,watch:0},loss:r.loss||{blocking:0,retailValue:0},cashOpening:r.cashOpening||{blocking:1,status:'NOT_STARTED',ready:0,lines:0,mismatch:0}}}}));
  const ready=rows.filter(x=>x.day.opening_status==='OPENED').length,
    blocked=rows.filter(x=>(x.opening.blockers>0||Number(x.cashOpening?.blocking||0)>0)&&x.day.opening_status!=='OPENED').length,
  ...
  <div class="network-section-title"><div><strong>Priorités réseau</strong><span>Classement par caisses d’ouverture, incidents, démarque, clôture caisse, prix/promo, DLC, stock et progression.</span></div></div>
  <div class="network-store-grid">${sorted.map(card).join('')}</div>`;
}

const risk=r=>Number(r.cashOpening?.mismatch||0)*240+Number(r.cashOpening?.blocking||0)*90+Number(r.sla?.escalated||0)*250+Number(r.handover?.blocking||0)*220+Number(r.commercial?.mismatch||0)*180+Number(r.loss?.blocking||0)*170+Number(r.loss?.pendingApproval||0)*120+Number(r.cash?.recounts||0)*160+Number(r.cash?.blocking||0)*80+Number(r.commercial?.pending||0)*60+Number(r.inventory?.pendingRecounts||0)*90+Number(r.inventory?.varianceLines||0)*20+(Number(r.dlc?.expired||0)+Number(r.dlc?.critical||0))*180+Number(r.sla?.overdue||0)*140+Number(r.criticalIncidents||0)*100+Number(r.opening.blockers||0)*30+Number(r.openIncidents||0)*8+(r.day.opening_status==='OPENED'?0:10);
function card(r){
  const open=r.day.opening_status==='OPENED',cashOpeningAlert=!open&&Number(r.cashOpening?.blocking||0)>0,cashAlert=Number(r.cash?.recounts||0)>0||(r.day.closing_status!=='CLOSED'&&r.cash?.status==='REVIEW'),lossAlert=Number(r.loss?.blocking||0)>0,danger=Number(r.sla?.escalated||0)>0||Number(r.sla?.overdue||0)>0||r.criticalIncidents>0||cashOpeningAlert||cashAlert||lossAlert||(!open&&r.opening.blockers>0);
  return`<article class="card network-store ${danger?'critical':''}">
    ...
    <div class="network-signals"><div><span>Caisses ouv.</span><strong>${r.cashOpening?.ready||0}/${r.cashOpening?.lines||0}</strong></div><div><span>Incidents</span><strong>${r.openIncidents}</strong></div><div><span>Démarque</span><strong>${r.loss?.blocking||0}</strong></div><div><span>DLC urgentes</span><strong>${Number(r.dlc?.expired||0)+Number(r.dlc?.critical||0)}</strong></div></div>
