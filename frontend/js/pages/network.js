...
const risk=r=>Number(r.sla?.escalated||0)*250+(Number(r.dlc?.expired||0)+Number(r.dlc?.critical||0))*180+Number(r.sla?.overdue||0)*140+Number(r.criticalIncidents||0)*100+Number(r.opening.blockers||0)*30+Number(r.openIncidents||0)*8+(r.day.opening_status==='OPENED'?0:10);
function card(r){
  ...
  return`<article class="card network-store ${danger?'critical':''}">
    ...
    <div class="network-signals"><div><span>Incidents</span><strong>${r.openIncidents}</strong></div><div><span>DLC urgentes</span><strong>${Number(r.dlc?.expired||0)+Number(r.dlc?.critical||0)}</strong></div><div><span>Actions DLC</span><strong>${r.dlc?.pendingActions||0}</strong></div><div><span>SLA retard</span><strong>${r.sla?.overdue||0}</strong></div></div>
