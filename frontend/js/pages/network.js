...
const risk=r=>Number(r.sla?.escalated||0)*250+Number(r.handover?.blocking||0)*220+Number(r.inventory?.pendingRecounts||0)*90+Number(r.inventory?.varianceLines||0)*20+(Number(r.dlc?.expired||0)+Number(r.dlc?.critical||0))*180+Number(r.sla?.overdue||0)*140+Number(r.criticalIncidents||0)*100+Number(r.opening.blockers||0)*30+Number(r.openIncidents||0)*8+(r.day.opening_status==='OPENED'?0:10);
function card(r){
