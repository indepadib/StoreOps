...
export async function renderInventory(){
  ...
  $('#inventoryContent').innerHTML=`
    ...
    <div class="inventory-overview">
      ...
      ${miniKpi('Prêts',s.readyToPost||0)}
    </div>
    <details class="card inventory-advanced" ${items.some(x=>x.status==='READY_TO_POST')?'open':''}>
      <summary><div><strong>Détails & inventaires avancés</strong><span>Sessions, inventaire complet, politique et posting Dynamics.</span></div><b>⌄</b></summary>
...
function quickPanel(express){
  const recount=express?.lines?.find(x=>x.status==='RECOUNT')||null;
  const metrics=express?.metrics||{lines:0,counted:0,pending:0,recounts:0};
  if(recount)return`<section class="card inventory-express inventory-express-recount">
    ...
    <div class="inventory-express-grid">
      ...
      <label><span>Motif de l’écart *</span><select id="invQuickReason"><option value="">Choisir le motif</option>${cfg.reasons.map(x=>`<option value="${x.code}">${esc(x.label)}</option>`).join('')}</select></label>
    </div>
    <button class="btn brand inventory-primary-cta" id="invQuickCount" data-ean="${esc(recount.ean)}">Valider le recomptage</button>
    <div class="inventory-express-progress"><span>${metrics.counted}/${metrics.lines} article(s) finalisé(s)</span><span>${metrics.recounts} recomptage(s) en attente</span></div>
  </section>`;
  ...
  return`<section class="card inventory-express">
    ...
    <div class="inventory-express-grid inventory-express-scan">
      ...
      <label class="inventory-reason-field"><span>Motif si vous anticipez un écart</span><select id="invQuickReason"><option value="">Aucun / à déterminer</option>${cfg.reasons.map(x=>`<option value="${x.code}">${esc(x.label)}</option>`).join('')}</select></label>
    </div>
    <button class="btn brand inventory-primary-cta" id="invQuickCount">Valider & article suivant</button>
    <div class="inventory-express-progress"><span>${express?`${metrics.counted}/${metrics.lines} article(s) finalisé(s)`:'Le premier scan démarre automatiquement la session.'}</span>${express&&metrics.lines&&metrics.pending===0?`<button class="btn soft" id="invQuickFinish" data-session="${esc(express.id)}">Terminer l’inventaire</button>`:''}</div>
  </section>`
}
...
function sessionCard(inv){
 ...
 return`<article class="card inventory-session ${ready?'inventory-ready':''}">
   ...
   <div class="inventory-footer"><div class="small muted">${inv.status==='POSTED'?`Posté par ${esc(inv.posted_by_name||'—')} · ${dt(inv.posted_at)}`:inv.status==='READY_TO_POST'?`Validé par ${esc(inv.reviewed_by_name||'—')} · ${dt(inv.reviewed_at)}`:pending?`${pending} ligne(s) restent à compter ou recomptabiliser.`:'Tous les écarts sont comptés.'}</div>
   <div class="row">${editable?`<button class="btn soft" data-finalize-inventory="${inv.id}" ${!inv.lines.length||pending?'disabled':''}>Valider la session</button>`:''}${ready&&canManage()?`<button class="btn brand" data-post-inventory="${inv.id}">Envoyer l’ajustement Dynamics</button>`:''}</div></div>
 </article>`}
...
async function quickCount(){
 ...
 try{
  const forced=btn?.dataset?.ean||'',ean=forced||$('#invQuickEan')?.value.trim(),qtyInput=$('#invQuickRecountQty')||$('#invQuickQty'),raw=qtyInput?.value,reasonCode=$('#invQuickReason')?.value||null;
  ...
  const result=await api(`/api/stores/${app.storeId}/inventory/express/count`,{method:'POST',body:JSON.stringify({ean,quantity,reasonCode})});
  toast(result.step==='RECOUNT_REQUIRED'?'Écart détecté : un recomptage est nécessaire.':'Comptage enregistré. Scanne l’article suivant.');
...
async function countLine(btn){if(btn?.disabled)return;try{const id=btn.dataset.countLine,input=document.querySelector(`[data-count-qty="${id}"]`),raw=input?.value;if(raw==null||String(raw).trim()==='')throw new Error('Saisis la quantité physique comptée.');const quantity=Number(raw),reasonCode=document.querySelector(`[data-count-reason="${id}"]`)?.value||null;if(!Number.isFinite(quantity)||quantity<0)throw new Error('Quantité physique invalide.');btn.disabled=true;await api(`/api/inventory/lines/${id}/count`,{method:'POST',body:JSON.stringify({quantity,reasonCode,recount:btn.dataset.recount==='1'})});toast('Comptage enregistré.');await renderInventory()}catch(e){toast(e.message);btn.disabled=false}}
async function finalize(id,btn,express=false){if(btn?.disabled)return;try{if(btn){btn.disabled=true;btn.textContent='Validation…'}const r=await api(`/api/inventory/${id}/finalize`,{method:'POST'});toast(r.highVarianceLines?.length?`Inventaire terminé · ${r.highVarianceLines.length} écart(s) à analyser.`:'Inventaire terminé et prêt à intégrer.');await renderInventory()}catch(e){toast(e.message);if(btn){btn.disabled=false;btn.textContent=express?'Terminer l’inventaire':'Valider la session'}}}
async function postInventory(id,btn){if(btn?.disabled)return;try{btn.disabled=true;btn.textContent='Envoi…';const r=await api(`/api/inventory/${id}/post`,{method:'POST'});toast(r.dynamics?.simulated?'Ajustement simulé : aucun write réel n’a été présenté comme effectué.':'Ajustement Dynamics confirmé.');await renderInventory()}catch(e){toast(e.message);btn.disabled=false;btn.textContent='Envoyer l’ajustement Dynamics'}}
