...
async function createSession(){const b=$('#createInventory');if(b?.disabled)return;try{if(b)b.disabled=true;const created=await api(`/api/stores/${app.storeId}/inventory`,{method:'POST',body:JSON.stringify({type:$('#invType').value,zone:$('#invZone').value.trim(),comment:$('#invComment').value.trim()})});if(created?.id){try{sessionStorage.setItem(FOCUS_INVENTORY_KEY,created.id)}catch{}}inventoryView='SESSIONS';toast('Inventaire avancé créé. Scanne maintenant les articles de cette session.');await renderInventory()}catch(e){toast(e.message)}finally{if(b)b.disabled=false}}
