...
const metricMapLabel=m=>Object.entries(m||{}).map(([u,v])=>`${Number(v)} ${u}`).join(' · ')||'0';
...
function quickPanel(express){
  ...
  if(explain)return`<section class="card inventory-express inventory-express-recount">
    ...
    <div class="inventory-session-kpis"><div><span>Théorique</span><strong>${explain.theoretical_qty}${explain.unit?` ${esc(explain.unit)}`:''}</strong></div><div><span>Compté</span><strong>${explain.final_qty}${explain.unit?` ${esc(explain.unit)}`:''}</strong></div><div><span>Écart</span><strong>${explain.final_variance}${explain.unit?` ${esc(explain.unit)}`:''}</strong><div class="small muted">${explain.variance_control_qty??'—'} ${esc(explain.variance_control_unit||'')}</div></div></div>
...
function conceptCard(){return`<div class="card"><div class="label">Règle de contrôle</div><strong>Écart ≥ ${cfg.policy.recount_qty_threshold} → recomptage</strong><div class="small muted" style="margin-top:5px">Écart ≥ ${cfg.policy.incident_qty_threshold} → incident Stock avec preuve. Seuils évalués en <b>pièce</b> pour les unités, <b>kg</b> pour les stocks g/kg et <b>L</b> pour mL/L. Le comptage reste saisi dans l’unité stock D365.</div></div>`}
function policyCard(){return`<div class="card inventory-policy"><div class="label">Politique réseau</div><div class="form-grid" style="margin-top:8px"><div class="field"><label>Recomptage dès écart de contrôle ≥</label><input id="invRecountThreshold" type="number" min="0" step="0.01" value="${cfg.policy.recount_qty_threshold}"></div><div class="field"><label>Incident dès écart de contrôle ≥</label><input id="invIncidentThreshold" type="number" min="0" step="0.01" value="${cfg.policy.incident_qty_threshold}"></div></div><div class="small muted" style="margin:8px 0">Base des seuils : pièce · kg pour g/kg · L pour mL/L. Le stock et le comptage restent dans l’unité D365. Comptage aveugle activé ; privilégier un autre compteur pour le recomptage.</div><button class="btn soft" id="saveInventoryPolicy">Enregistrer la politique</button></div>`}
...
function lineRow(l){
 ...
 const varianceHtml=view.blind?hiddenValue():`${variance??'—'}${variance!=null&&l.unit?` ${esc(l.unit)}`:''}${variance!=null&&l.variance_control_unit&&String(l.variance_control_unit)!==String(l.unit||'')?`<div class="small muted">${l.variance_control_qty} ${esc(l.variance_control_unit)} contrôle</div>`:''}`;
