...
function sessionCard(inv){
 ...
 return`<article class="card inventory-session ${ready?'inventory-ready':''}">
   ...
   <div class="row">${editable?`<button class="btn soft" data-finalize-inventory="${inv.id}" ${!inv.lines.length||pending?'disabled':''}>Valider la session</button>`:''}${ready&&canManage()?`<button class="btn brand" data-export-inventory="${inv.id}">Exporter Excel</button>`:''}</div></div>
 </article>`}
