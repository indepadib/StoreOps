...
function bind(){
 ...
 document.querySelectorAll('[data-check-cash-opening]').forEach(b=>b.addEventListener('click',async()=>{try{const id=b.dataset.checkCashOpening,payload={tillCode:b.dataset.tillCode,cashierName:val(id,'cashier'),declaredFloat:Number(val(id,'float')),posOk:checked(id,'pos_ok'),tpeOk:checked(id,'tpe_ok'),printerOk:checked(id,'printer_ok'),shiftOpened:checked(id,'shift_opened'),note:val(id,'note')||''};await api(`/api/stores/${app.storeId}/cash-opening/check`,{method:'POST',body:payload});toast('Caisse prête.');renderCashOpening()}catch(e){toast(Array.isArray(e.details)&&e.details.length?e.details.join(' · '):e.message);renderCashOpening()}}));
