...
export function isControlling(){return app.user?.permissions_profile==='controlling'}
export function isExecutive(){return app.user?.permissions_profile==='executive'}
export function isNetworkReadOnly(){return isControlling()||isExecutive()}
...
export function accessProfileLabel(){
 if(isPlatformAdmin())return'Administrateur StoreOps';
 if(isQualityAudit())return'Qualité & audit';
 if(isControlling())return'Contrôle de gestion';
 if(isExecutive())return'Dirigeant / Finance';
 if(app.user?.permissions_profile==='development')return'Développement réseau';
 if(app.user?.role==='ops_director')return'Direction d’exploitation';
 if(app.user?.role==='store_manager')return'Responsable magasin';
 return'Utilisateur magasin'
}
