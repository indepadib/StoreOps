...
export function accessProfileLabel(){if(isPlatformAdmin())return'Administrateur StoreOps';if(isQualityAudit())return'Qualité & audit';if(app.user?.permissions_profile==='management_control')return'Contrôle de gestion';if(app.user?.permissions_profile==='development')return'Développement réseau';if(app.user?.role==='ops_director')return'Direction d’exploitation';if(app.user?.role==='store_manager')return'Responsable magasin';return'Utilisateur magasin'}
