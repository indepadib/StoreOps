...
function ensureComponentCss(){if(document.querySelector('link[data-ux185-components]'))return;const l=document.createElement('link');l.rel='stylesheet';l.href='/ux-v185-components.css?v=1850';l.dataset.ux185Components='1';document.head.appendChild(l)}
...
function ensureAvatar(){const controls=document.querySelector('.top-controls');if(!controls)return;let badge=document.getElementById('uxUserBadge');if(!badge){badge=document.createElement('button');badge.id='uxUserBadge';badge.type='button';badge.className='ux-user-badge';controls.appendChild(badge)}badge.title=app.user?.name||'Profil';badge.innerHTML=`<span>${initials()}</span><small>${String(app.user?.name||'Profil')}</small>`}
...
function refreshShell(){ensureComponentCss();roleClass();ensurePalette();ensureCommandTrigger();ensureAvatar();simplifyDirectorNav()}
