...
function hide(el){if(el)el.classList.add('manager-dlc-hidden')}
function show(el){if(el)el.classList.remove('manager-dlc-hidden')}
...
function compactEntry(entry){
  ...
  if(heading){const strong=heading.querySelector('strong');if(strong)strong.textContent='Contrôler un article';const small=heading.querySelector('.small');if(small)small.textContent='Scannez ou saisissez l’EAN, puis renseignez seulement les informations utiles.'}
  ...
  const save=entry.querySelector('#saveDlc');if(save)save.textContent='Enregistrer le contrôle';
}
...
function simplifyPage(){
  ...
  resetHidden();
  const tb=titleBlock();if(tb){const h=tb.querySelector('h1');if(h)h.textContent='DLC / DDM';const p=tb.querySelector('p');if(p)p.textContent='Contrôler les dates et traiter les lots à risque.'}
  hide(c.querySelector('.dlc-kpis'));
  const grids=[...c.querySelectorAll(':scope > .grid.g3')];grids.forEach(hide);
  ...
  const entry=c.querySelector('.dlc-entry'),queueTitle=sections.find(x=>x.textContent.includes('File de traitement DLC')),
        queue=c.querySelector('.dlc-priority-list');
  document.querySelector('#managerDlcQueueHead')?.remove();
  const home=managerHome();
  if(mode==='home'){
    show(home);hide(entry);hide(queueTitle);hide(queue);
  }else if(mode==='new'){
    hide(home);show(entry);compactEntry(entry);hide(queueTitle);hide(queue);
    ...
    const pending=pendingCards();queueHeader(pending.length);focusQueue(pending);
  }
}
...
export function initManagerDlcFocus(){watchActions();enhance();new MutationObserver(schedule).observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class']})}
