...
const categoryRank={OPENING:0,COMMERCIAL:1,STOCK:2,RECEIPT:3,INVENTORY:4,DLC:5,QUALITY:6,LOSS:7,OTHER:8};
...
function showcaseStockSignals(){
  if(!app.showcase||app.storeId!=='val-fleuri')return[];
  return [
    {id:'stock-neg-coca',type:'NEGATIVE',product:'Coca-Cola 1,5L',ean:'5449000000996',qty:-3,detail:'Stock système -3 · vérifier rayon + réserve puis corriger l’écart'},
    {id:'stock-oos-eau',type:'OUT',product:'Sidi Ali 1,5L',ean:'6111000000011',qty:0,detail:'Rupture détectée · stock disponible 0 · contrôler réserve / facing'},
    {id:'stock-oos-lait',type:'OUT',product:'Lait UHT entier 1L',ean:'6111035000013',qty:0,detail:'Rupture détectée · vente récente mais aucun stock disponible'}
  ];
}

async function loadStockSignals(){
  const live=await safe(api(`/api/stores/${app.storeId}/stock-signals`),null);
  if(live?.items)return live.items;
  return showcaseStockSignals();
}

function action({id,category,severity='HIGH',title,detail,page,blocking=false,meta='',count=1}){
  return {id,category,severity,title,detail,page,blocking:!!blocking,meta,count};
}

function sortActions(rows){
  return rows.sort((a,b)=>Number(b.blocking)-Number(a.blocking)||(severityRank[a.severity]??9)-(severityRank[b.severity]??9)||(categoryRank[a.category]??9)-(categoryRank[b.category]??9));
}

export async function loadManagerInbox(){
  const [dashboard,commercial,receiptRows,inventoryData,lossData,incidentData,staffData,coldData,cashOpenData,qualityRows,stockSignals]=await Promise.all([
  ...
  if(n(dashboard.handover?.blocking)>0)items.push(action({id:'handover-blocking',category:'OPENING',severity:'CRITICAL',title:'Passation bloquante',detail:`${dashboard.handover.blocking} sujet(s) doivent être traités avant l’ouverture`,page:'handover',blocking:true}));
  if(n(staff.blocking)>0)items.push(action({id:'staffing-opening',category:'OPENING',severity:'CRITICAL',title:'Équipe d’ouverture à compléter',detail:`${staff.pending||0} personne(s) restent à pointer / confirmer`,page:'staffing',blocking:true}));
  if(n(cold.blocking)>0)items.push(action({id:'cold-opening',category:'OPENING',severity:'CRITICAL',title:'Chaîne du froid à valider',detail:`${cold.blocking} zone(s) restent non validées`,page:'coldChain',blocking:true}));
  if(n(cashOpen.blocking)>0)items.push(action({id:'cash-opening',category:'OPENING',severity:'CRITICAL',title:'Caisses à préparer',detail:`${cashOpen.blocking} caisse(s) ne sont pas encore prêtes`,page:'cashOpening',blocking:true}));

  for(const row of commercial.items||[]){
    if(row.status==='VERIFIED')continue;
    items.push(action({id:`commercial-${row.id}`,category:'COMMERCIAL',severity:row.status==='MISMATCH'?'CRITICAL':row.priority||'HIGH',title:commercialTitle(row),detail:commercialDetail(row),page:'commercial',blocking:!!row.blocking_opening,meta:row.status==='MISMATCH'?'Écart détecté':'À valider'}));
  }
  ...
  for(const receipt of receiptRows||[]){
    ...
    for(const line of receipt.lines||[]){
      ...
      items.push(action({id:`receipt-${receipt.id}-${line.id}`,category:'RECEIPT',severity:'HIGH',title:`Réception · ${line.product_name||line.description||line.item_name||line.ean||'Article à contrôler'}`,detail:`${receipt.po_number||receipt.purchase_order||receipt.id||'Réception'} · quantité / qualité à valider`,page:'receipts',blocking:true,meta:'Article à valider'}));
    }
  }

  for(const sig of stockSignals||[]){
    const negative=sig.type==='NEGATIVE'||n(sig.qty)<0;
    items.push(action({id:`stock-${sig.id||sig.ean||sig.product}`,category:'STOCK',severity:negative?'CRITICAL':'HIGH',title:`${negative?'Stock négatif':'Rupture'} · ${sig.product||sig.product_name||sig.ean||'Article'}`,detail:sig.detail||`${negative?'Stock négatif':'Stock disponible 0'} · contrôle rayon/réserve requis`,page:'inventory',blocking:negative,meta:sig.ean?`EAN ${sig.ean}`:''}));
  }

  for(const inv of inventoryData.items||[]){
    if(inv.status==='READY_TO_POST')items.push(action({id:`inv-post-${inv.id}`,category:'INVENTORY',severity:'HIGH',title:'Inventaire à valider / poster',detail:`${inv.zone||'Périmètre'} · ${inv.metrics?.varianceLines||0} ligne(s) en écart`,page:'inventory',meta:'Validation Responsable'}));
    else if(n(inv.metrics?.recounts)>0)items.push(action({id:`inv-recount-${inv.id}`,category:'INVENTORY',severity:'HIGH',title:'Recomptage stock requis',detail:`${inv.metrics.recounts} article(s) doivent être recomptés`,page:'inventory'}));
  }

  const dlcCritical=n(dashboard.dlc?.expired)+n(dashboard.dlc?.critical);
  if(dlcCritical>0)items.push(action({id:'dlc-critical',category:'DLC',severity:'CRITICAL',title:'DLC / DDM prioritaires',detail:`${dlcCritical} lot(s) périmés ou critiques à traiter`,page:'dlc',blocking:true,count:dlcCritical}));
  else if(n(dashboard.dlcAtRisk)>0)items.push(action({id:'dlc-risk',category:'DLC',severity:'HIGH',title:'DLC / DDM à contrôler',detail:`${dashboard.dlcAtRisk} lot(s) à risque`,page:'dlc',count:dashboard.dlcAtRisk}));

  if(n(quality.nonConform)>0)items.push(action({id:'quality-nc',category:'QUALITY',severity:n(quality.temperatureNok)>0?'CRITICAL':'HIGH',title:'Non-conformités qualité',detail:`${quality.nonConform} contrôle(s) non conformes${quality.temperatureNok?` · ${quality.temperatureNok} température(s) NOK`:''}`,page:'quality',blocking:n(quality.temperatureNok)>0}));
  if(n(loss.blocking)>0)items.push(action({id:'loss-blocking',category:'LOSS',severity:'HIGH',title:'Démarque à finaliser',detail:`${loss.blocking} sortie(s) restent à justifier / valider`,page:'losses',blocking:true}));

  if((dashboard.day?.opening_status||'NOT_STARTED')!=='OPENED'&&!items.some(x=>x.category==='OPENING'||x.blocking)){
    items.push(action({id:'opening-flow',category:'OPENING',severity:'NORMAL',title:'Continuer le parcours d’ouverture',detail:`${dashboard.opening?.done||0}/${dashboard.opening?.total||0} étapes validées`,page:'opening'}));
  }

  sortActions(items);
  const critical=items.filter(x=>x.severity==='CRITICAL').length,blocking=items.filter(x=>x.blocking).length;
  const alertCritical=alerts.filter(x=>x.criticality==='CRITICAL').length;
  return {dashboard,commercial,receiptRows,inventoryData,lossData,incidentData,staff,cold,cashOpen,receipts,quality,maintenance,stockSignals,items,alerts,summary:{total:items.length,critical,blocking,alerts:alerts.length,alertCritical}};
}

export function actionKind(item){return item.severity==='CRITICAL'?'danger':item.severity==='HIGH'?'warn':'neutral'}
...
export function syncManagerNav(inbox){
  ...
  const today=byPage('today'),validate=byPage('managerControls'),alerts=byPage('incidents'),journey=byPage('managerJourney'),more=byPage('managerMore');
  if(today)today.textContent='Aujourd’hui';
  ...
  [today,validate,alerts,journey,more].filter(Boolean).forEach(x=>nav.appendChild(x));
}
