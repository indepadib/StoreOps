...
const adminEntries={
  ...
  integrations:{section:'integrationsStudioSection',modules:['./admin-studio-integrations.js','./admin-validation-center.js','./admin-d365-mapping.js','./admin-price-history-mapping.js','./admin-loss-export-mapping.js','./admin-taxonomy-mapping.js']},
...
async function importAdmin(path){if(adminModules.has(path))return adminModules.get(path);const mod=await import(`${path}?v=${BUILD}`);adminLoaded.add(path);adminModules.set(path,mod);return mod}
...
async function openAdminEntry(key,label){const cfg=adminEntries[key],root=document.getElementById('adminStudioContent');if(!cfg||!root)return;let host=document.getElementById('studioExtensions');if(!host){host=document.createElement('div');host.id='studioExtensions';root.appendChild(host)}host.innerHTML=`<div class="card" id="adminLazyLoading"><div class="label">ADMIN STUDIO</div><strong>Chargement de ${label}…</strong><div class="small muted" style="margin-top:6px">Seul ce module est chargé. Le reste reste au repos.</div></div>`;try{for(const path of cfg.modules){const mod=await importAdmin(path);if(typeof mod?.mountAdminSection==='function')await mod.mountAdminSection()}const section=await waitFor(cfg.section,5000);document.getElementById('adminLazyLoading')?.remove();section.scrollIntoView({behavior:'smooth',block:'start'})}catch(e){host.innerHTML=`<div class="banner ban-danger"><strong>Module Admin indisponible</strong><span>${String(e?.message||e)}</span></div>`;console.error(e)}}
