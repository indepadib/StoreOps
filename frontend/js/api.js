...
async function liveApi(path,options={}){const {noDedupe,...fetchOptions}=options;const headers=applyAuth({'content-type':'application/json',...(fetchOptions.headers||{})}),url=apiUrl(path);let r;try{r=await fetch(url,{...fetchOptions,headers})}catch{const e=new Error(`Impossible de joindre l'API StoreOps. Vérifie STOREOPS_API_BASE et que le backend est déployé. URL : ${url}`);e.code='API_UNREACHABLE';throw e}const data=await parseJsonResponse(r,url);if(!r.ok){const e=new Error(data.error||`Erreur HTTP ${r.status}`);e.status=r.status;e.code=data.code;e.details=data.details||data.issues;throw e}return data}
export async function api(path,options={}){
  if(isShowcase())return showcaseApi(path,options);
  const method=String(options.method||'GET').toUpperCase();
  if(method!=='GET'||options.noDedupe||options.signal)return liveApi(path,options);
  const key=String(path);
  if(inflightGets.has(key))return inflightGets.get(key);
  const request=liveApi(path,options).finally(()=>inflightGets.delete(key));
...
export async function health({force=false}={}){
  ...
  if(!force&&window.STOREOPS_BOOT_HEALTH)return window.STOREOPS_BOOT_HEALTH;
  if(!force&&healthPromise)return healthPromise;
  const request=(async()=>{const url=apiUrl('/api/health');let r;try{r=await fetch(url,{cache:'no-store'})}catch{const e=new Error(`Impossible de joindre l'API StoreOps. Configure STOREOPS_API_BASE dans Netlify puis redéploie. URL : ${url}`);e.code='API_UNREACHABLE';throw e}const data=await parseJsonResponse(r,url);if(!r.ok){const e=new Error(data.error||`Healthcheck API en erreur (${r.status}).`);e.status=r.status;throw e}return data})();
  if(!force)healthPromise=request;
  try{return await request}finally{if(!force)healthPromise=null}
}
