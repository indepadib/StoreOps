...
function hasHeader(headers,name){return Object.keys(headers||{}).some(k=>k.toLowerCase()===name.toLowerCase())}
function isNativeBody(body){
  return (typeof FormData!=='undefined'&&body instanceof FormData)||(typeof Blob!=='undefined'&&body instanceof Blob)||(typeof ArrayBuffer!=='undefined'&&(body instanceof ArrayBuffer||ArrayBuffer.isView?.(body)))||(typeof URLSearchParams!=='undefined'&&body instanceof URLSearchParams)||(typeof ReadableStream!=='undefined'&&body instanceof ReadableStream)
}
function prepareOptions(options={}){
  const prepared={...options},headers={...(options.headers||{})};
  if(prepared.body!==undefined&&prepared.body!==null&&!isNativeBody(prepared.body)){
    if(typeof prepared.body!=='string')prepared.body=JSON.stringify(prepared.body);
    if(!hasHeader(headers,'content-type'))headers['content-type']='application/json';
  }
  prepared.headers=applyAuth(headers);
  return prepared
}
...
export async function api(path,options={}){const prepared=prepareOptions(options);if(isShowcase())return showcaseApi(path,prepared);const url=apiUrl(path);let r;try{r=await fetch(url,prepared)}catch{const e=new Error(`Impossible de joindre l'API StoreOps. Vérifie STOREOPS_API_BASE et que le backend est déployé. URL : ${url}`);e.code='API_UNREACHABLE';throw e}const data=await parseJsonResponse(r,url);if(!r.ok){const e=new Error(data.error||`Erreur HTTP ${r.status}`);e.status=r.status;e.code=data.code;e.details=data.details||data.issues;throw e}return data}
export async function health(){if(isShowcase()){const h=await mockApi('/api/health');return{...h,version:SHOWCASE_VERSION}}const url=apiUrl('/api/health');let r;try{r=await fetch(url,{cache:'no-store'})}catch{const e=new Error(`Impossible de joindre l'API StoreOps. Configure STOREOPS_API_BASE dans Netlify puis redéploie. URL : ${url}`);e.code='API_UNREACHABLE';throw e}const data=await parseJsonResponse(r,url);if(!r.ok){const e=new Error(data.error||`Healthcheck API en erreur (${r.status}).`);e.status=r.status;throw e}return data}
