...
async function parseJsonResponse(r,url){
  ...
  if(!type.includes('application/json')){
    ...
    const hint=preview.startsWith('<')||type.includes('text/html')
      ?`StoreOps attend du JSON mais reçoit une page HTML. Vérifie STOREOPS_API_BASE côté Netlify : il doit pointer vers l'origine publique du backend (ex. https://api-storeops.example.com), sans /api à la fin.`
