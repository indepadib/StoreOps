...
async function renderPage(page){try{if(page==='today')return renderToday();if(page==='opening')return renderProcess('opening');if(page==='closing')return renderProcess('closing');if(page==='dlc')return renderDlc();if(page==='receipts')return renderReceipts();if(page==='inventory')return renderInventory();if(page==='quality')return renderQuality();if(page==='incidents')return renderIncidents();if(page==='network')return renderNetwork();if(page==='system')return renderSystem()}catch(e){console.error(e);toast(e.message)}}
function bind(){
