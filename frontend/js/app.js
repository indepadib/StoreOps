...
async function renderClosingPage(){await renderProcess('closing');try{const d=await api(`/api/stores/${app.storeId}/dashboard`);if(d.cash?.blocking){$('#closingContent')?.insertAdjacentHTML('afterbegin',`<div class="banner ban-danger process-lock cash-closing-gate"><strong>Clôture caisses à finaliser</strong><span>Statut : ${d.cash.status||'non démarrée'} · ${d.cash.pending||0} shift(s) à compter · ${d.cash.recounts||0} recomptage(s). Le magasin ne peut pas être fermé avant validation.</span><button class="btn soft" data-go-cash style="margin-top:8px">Ouvrir Caisses & clôture</button></div>`)}}catch{}}
async function renderPage(page){try{if(page==='today')return renderToday();if(page==='opening')return renderProcess('opening');if(page==='closing')return renderClosingPage();if(page==='commercial')return renderCommercial();if(page==='dlc')return renderDlc();if(page==='receipts')return renderReceipts();if(page==='inventory')return renderInventory();if(page==='quality')return renderQuality();if(page==='incidents')return renderIncidents();if(page==='cash')return renderCash();if(page==='network')return renderNetwork();if(page==='system')return renderSystem()}catch(e){console.error(e);toast(e.message)}}
function bind(){
