...
import { renderCashOpening } from './pages/cash-opening.js';
...
import { renderLosses } from './pages/losses.js';
...
async function renderOpeningPage(){await renderProcess('opening');try{const c=await api(`/api/stores/${app.storeId}/cash-opening`);if(c.summary?.blocking){$('#openingContent')?.insertAdjacentHTML('afterbegin',`<div class="banner ban-danger process-lock"><strong>${c.summary.blocking} caisse(s) à préparer avant ouverture</strong><span>${c.summary.ready||0}/${c.summary.lines||0} prête(s) · ${c.summary.mismatch||0} non conforme(s). Affectation, fond, POS, TPE, imprimante et shift Dynamics doivent être validés.</span><button class="btn soft" data-go-cash-opening style="margin-top:8px">Ouvrir Préparation caisses</button></div>`)}else if(c.summary?.status==='READY'){$('#openingContent')?.insertAdjacentHTML('afterbegin',`<div class="banner ban-info process-lock"><strong>Caisses prêtes</strong><span>${c.summary.ready||0}/${c.summary.lines||0} caisses conformes. L’étape 7 est validée automatiquement.</span></div>`)}}catch{}}
async function renderClosingPage(){await renderProcess('closing');try{const [d,l]=await Promise.all([api(`/api/stores/${app.storeId}/dashboard`),api(`/api/stores/${app.storeId}/losses`)]);if(d.cash?.blocking){$('#closingContent')?.insertAdjacentHTML('afterbegin',`<div class="banner ban-danger process-lock cash-closing-gate"><strong>Clôture caisses à finaliser</strong><span>Statut : ${d.cash.status||'non démarrée'} · ${d.cash.pending||0} shift(s) à compter · ${d.cash.recounts||0} recomptage(s). Le magasin ne peut pas être fermé avant validation.</span><button class="btn soft" data-go-cash style="margin-top:8px">Ouvrir Caisses & clôture</button></div>`)}if(l.summary?.blocking){$('#closingContent')?.insertAdjacentHTML('afterbegin',`<div class="banner ban-danger process-lock"><strong>${l.summary.blocking} perte(s) / démarque(s) à finaliser</strong><span>Chaque sortie du jour doit être documentée, approuvée si nécessaire et postée dans Dynamics avant fermeture.</span><button class="btn soft" data-go-losses style="margin-top:8px">Ouvrir Démarque & pertes</button></div>`)}}catch{}}
async function renderPage(page){try{if(page==='today')return renderToday();if(page==='opening')return renderOpeningPage();if(page==='cashOpening')return renderCashOpening();if(page==='closing')return renderClosingPage();if(page==='commercial')return renderCommercial();if(page==='dlc')return renderDlc();if(page==='receipts')return renderReceipts();if(page==='inventory')return renderInventory();if(page==='losses')return renderLosses();if(page==='quality')return renderQuality();if(page==='incidents')return renderIncidents();if(page==='cash')return renderCash();if(page==='network')return renderNetwork();if(page==='system')return renderSystem()}catch(e){console.error(e);toast(e.message)}}
function bind(){
  ...
  document.addEventListener('click',async e=>{try{
    const goCashOpening=e.target.closest('[data-go-cash-opening]');if(goCashOpening)return setPage('cashOpening');
    const goCash=e.target.closest('[data-go-cash]');if(goCash)return setPage('cash');
    const goLosses=e.target.closest('[data-go-losses]');if(goLosses)return setPage('losses');
